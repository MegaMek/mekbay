...
describe('UnitInitializerService', () => {
    ...
    it('synthesizes mounted equipment for critical-only equipment', () => {
        ...
        const caseII = inventory.find(entry => entry.id === 'CLCASEII@LT#11');

        expect(inventory.map(entry => entry.id)).toEqual(['CLMASC@LT#7', 'Supercharger@RT#10', 'CLCASEII@LT#11']);
        ...
        expect(caseII?.equipment?.flags.has('F_CASE_II')).toBeTrue();
    });
