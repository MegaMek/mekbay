...
export class ForcePilotEditorService {
  ...
  async editPerson(force: Force, personId: string): Promise<void> {
    ...
    if (force.gameSystem === GameSystem.AS) {
      ...
      const health = assignment
        ? cbtForce.getUnitSnapshot(assignment.unitId)?.query.crewState(asCrewPositionId(assignment.positionId))
        : CrewMember.from(person.health);
      const ref = this.dialogs.createDialog<EditPilotResult | null, EditPilotDialogComponent, EditPilotDialogData>(
        EditPilotDialogComponent,
        {
          data: {
            ...
            editPortrait: true,
            editWounds: true,
            unitTracksWounds: skillFacts ? unitTracksPilotWounds(skillFacts.unitType) : undefined,
            crew: [
              {
                id: asCrewPositionId(assignment?.positionId ?? 'reserve'),
                name: person.name ?? '',
                notes: person.notes,
            ...
            ...(isCBTForceMember(member) && skillFacts
              ? {
                  ...
                  preSkillBv:
                    (member.currentBaseBattleValue() ?? member.entity.battleValue()) +
                    (member.tagBattleValue() ?? 0) +
                    (member.c3BattleValue() ?? 0),
      ...
      if (!result) return;
      if (result.action) {
        await this.applyAction(force, personId, result.action);
        return;
      }
      const edited = result.crew[0];
      if (!edited) return;
      const applied = await this.savePerson(force, personId, {
      ...
      if (applied && edited.wounds !== undefined && edited.wounds !== (health?.wounds ?? 0)) {
        const currentAssignment = force.personnel().assignments.find((candidate) => candidate.personId === personId);
        if (currentAssignment) {
          const snapshot = cbtForce.getUnitSnapshot(currentAssignment.unitId);
          if (!snapshot) {
            this.failed();
            return;
          }
          const positionId = asCrewPositionId(currentAssignment.positionId);
          const current = snapshot.query.crewState(positionId);
          const saved = await cbtForce.dispatchUnitCommand(
            currentAssignment.unitId,
            {
              type: 'set-crew-state',
              positionId,
              wounds: edited.wounds,
              unconscious: current.unconscious,
              ejected: current.ejected,
            },
            snapshot.editContext,
          );
          if (!saved.accepted) this.failed();
        } else {
          const current = CrewMember.from(
            force.personnel().people.find((candidate) => candidate.id === personId)?.health,
          );
          const updated = current.withWoundTrackedState({ ...current, wounds: edited.wounds });
          await this.savePerson(force, personId, {
            health: updated.isPristine() ? undefined : updated.toRuntimeState(),
          });
        }
      }
    }
  ...
  private async applyAction(force: Force, personId: string, action: CrewEditAction): Promise<void> {
    ...
    const applied =
      action === 'delete'
        ? await force.deletePerson(personId)
        : assignment
          ? await force.unassignPerson(assignment.unitId, assignment.positionId)
          : false;
  ...
  private async editAlphaStrikePerson(force: Force, person: ForcePerson, unit?: ASForceUnit): Promise<void> {
    ...
    const ref = this.dialogs.createDialog<EditASPilotResult | null, EditASPilotDialogComponent, EditASPilotDialogData>(
      ...
      {
        data: {
          unitId: unit?.id ?? person.id,
          name: person.name ?? '',
          notes: person.notes,
          portrait: person.portrait,
          editNotes: true,
          editPortrait: true,
          skill: person.gunnery ?? 4,
          abilities: structuredClone([...(person.abilities ?? [])]),
          ...
          era: force.era(),
          unitType: summary?.type,
          unitSubtype: summary?.subtype,
          unitTypeCode: summary?.as?.TP,
          basePv: summary?.as?.PV,
        },
      },
    );
    const result = await firstValueFrom(ref.closed);
    if (!result) return;
    if (result.action) {
      await this.applyAction(force, person.id, result.action);
      return;
    }
    const applied = await this.savePerson(force, person.id, {
      name: result.name.trim() || undefined,
      notes: result.notes || undefined,
      portrait: result.portrait,
      gunnery: result.skill,
  ...
  private failed(): void {
    this.toast.showToast('The crew change could not be saved.', 'error');
  }
}
