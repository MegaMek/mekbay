...
export class CustomUnitsService {
  ...
  readonly pendingRecords = computed(() =>
    this.recordsValue().filter((r) => r.accountUuid === this.accountUuid() && r.pending),
  );
  onLocalChange?: () => void;
  ...
  private readonly broadcast =
    typeof BroadcastChannel === 'undefined' ? undefined : new BroadcastChannel('mekbay-custom-units');
  ...
  constructor() {
    ...
    if (this.broadcast)
      this.broadcast.onmessage = ({ data }) => {
        if (
          !Array.isArray(data) ||
          !data.every((key) => typeof key?.uuid === 'string' && typeof key?.accountUuid === 'string')
        )
          return;
        void this.enqueue(async () => {
          await this.initialize();
          await this.updateRecords(
            data.map(({ uuid, accountUuid }) => ({
              uuid,
              accountUuid,
              update: (current: SavedCustomUnit | undefined) => current,
            })),
          );
        ...
        await this.updateRecords([
          {
            ...
            update: (current) => {
              if (current)
                bound = { ...current, accountUuid: account, owned: true, pending: current.pending ?? 'save' };
              return undefined;
            },
          },
          {
            ...
            accountUuid: account,
            update: (current) => {
              if (bound && current && current.source !== bound.source)
                throw new Error('This custom unit identity is already in use.');
  ...
  save(entity: BaseEntity, options: SaveCustomUnitOptions = {}): Promise<SavedCustomUnit> {
    ...
    detached.fluffImageEncoded.set('');
    detached.iconEncoded.set('');
    ...
    return this.enqueue(async () => {
      ...
      if (!existing && this.recordsValue().some((r) => r.uuid === uuid))
        throw new Error('This custom unit identity is already in use.');
      if (!existing && options.originalUnitUuid) detached.refitFromUUID.set(options.originalUnitUuid);
      const originalUnitUuid = detached.refitFromUUID();
      ...
      const [hashed] = await this.updateRecords(
        [
          {
            uuid,
            accountUuid: account,
            update: (current) => {
              if (current?.source !== existing?.source || current?.pending === 'delete') {
                throw new Error(
                  'This design changed in another tab. Your draft is still open; save it as a new design.',
                );
              }
              return {
                ...record,
                hash: source.hash,
                ...(current?.syncedHash ? { syncedHash: current.syncedHash } : {}),
              };
            },
          },
        ],
      ...
      await this.updateRecords([
        {
          ...
          update: (current) => (current ? { ...current, pending: 'delete' } : undefined),
        },
      ]);
      this.onLocalChange?.();
    });
  ...
  async prepareSummaries(
    ...
    options: UnitSummaryProjectionOptions & { readonly cachedOnly?: boolean } = {
    ...
    const keys = new Set(
      this.recordsValue()
        .filter((record) => record.pending !== 'delete')
        .map((record) => this.sourceKey(record)),
    );
    ...
    for (const record of records) {
      ...
      if (
        cached &&
        cached.dependencies === dependencyKey &&
        cached.summary.hash === source.hash &&
        cached.summary.summaryVersion === UNIT_SUMMARY_VERSION
      )
        prepared.set(key, cached.summary);
      else rebuilds.push({ record, source, key });
    }
    if (rebuilds.length && !options.cachedOnly) {
      const outcomes = await projectUnitSummaryBatches(
        rebuilds.map(
          ({ record, source }) =>
            async () =>
              this.projectionInput(record, source),
        ),
        {
          projector: await dependencies.getProjector(),
          dependencies: dependencies.bundle,
          workers: this.summaryWorkers,
        },
        options,
      );
      options.signal.throwIfAborted();
  ...
  captureSources(): ReadonlyMap<UnitUuid, StoredCoreContent> {
    return new Map(
      this.records().flatMap((r) => {
        const source = this.sources.get(this.sourceKey(r));
        return source ? [[r.uuid, source] as const] : [];
      }),
    );
  }

  isOwned(uuid: UnitUuid): boolean {
    const record = this.recordsByUuid().get(uuid);
    return !!record && record.owned !== false;
  }

  hasUpdate(uuid: UnitUuid, sourceHash: string | undefined): boolean {
    const record = this.recordsByUuid().get(uuid);
    return !!record?.hash && record.hash !== sourceHash;
  }
  ...
  async acceptRemoteBatch(values: readonly SavedCustomUnit[], persist = true): Promise<void> {
    await this.enqueue(async () => {
        ...
        await this.updateRecords(
          records.map((record) => ({
            ...
            update: (current) => {
              if (
                current?.pending ||
                (current && current.owned !== false && record.owned === false) ||
                (current && current.updatedAt > record.updatedAt)
              )
                return current;
              if (
                current?.source === record.source &&
                current?.owned === record.owned &&
                current?.subscribed === record.subscribed &&
                current?.ownerId === record.ownerId &&
                current?.originalUnitUuid === record.originalUnitUuid
              )
                return current;
              return record;
            },
      ...
      for (const key of keys) {
        if (persist) this.temporary.delete(key);
        else this.temporary.add(key);
      }
  ...
  acknowledgeUpload(uploaded: SavedCustomUnit, hash: string, ownerId: string): Promise<void> {
    return this.enqueue(async () => {
      await this.updateRecords([
        {
          uuid: uploaded.uuid,
          accountUuid: uploaded.accountUuid ?? '',
          update: (current) => {
            ...
            return {
              ...current,
              hash: unchanged ? hash : current.hash,
              syncedHash: hash,
              ownerId,
  ...
  private async updateRecords(
    changes: Parameters<DbService['updateCustomUnits']>[0],
    verifiedSources?: ReadonlyMap<string, StoredCoreContent>,
  ): Promise<readonly (SavedCustomUnit | undefined)[]> {
    let changed = false;
    const records = await this.db.updateCustomUnits(
      changes.map((change) => ({
        ...change,
        update: (current) => {
    ...
    for (const [index, value] of records.entries()) {
      ...
      const affectsCatalog =
        record?.source !== old?.source ||
        record?.originalUnitUuid !== old?.originalUnitUuid ||
        (record?.pending === 'delete') !== (old?.pending === 'delete') ||
        record?.owned !== old?.owned ||
    ...
    if (changed) {
      try {
        this.broadcast?.postMessage(changes.map(({ uuid, accountUuid }) => ({ uuid, accountUuid })));
      } catch {
        /* Closing a tab cannot undo its saved edits. */
      }
    }
    return records;
  }
  ...
  private async hydrate(): Promise<void> {
    ...
    for (const value of nativeRecords) {
      try {
        ...
        if (
          existing &&
          ((existing.owned !== false && record.owned === false) ||
            ((existing.owned !== false) === (record.owned !== false) && existing.updatedAt >= record.updatedAt))
        )
          continue;
        const source = await this.sourceFor(record);
  ...
  private async sourceFor(record: SavedCustomUnit): Promise<StoredCoreContent> {
    ...
    return Object.freeze({
      file: makeUnitFileName(record.uuid, record.format),
      format: record.format,
      hash: asSourceHash(await sha1Base64Url(bytes)),
      bytes,
    });
  }
  ...
  private projectionInput(record: SavedCustomUnit, source: StoredCoreContent): UnitSummaryProjectionInput {
    return {
      entryKey: {
        origin: 'user',
        design: { provider: CUSTOM_UNIT_PROVIDER_ID, uuid: record.uuid },
  ...
  private async project(
    record: SavedCustomUnit,
    source: StoredCoreContent,
    dependencies: PreparedApplicationCatalogDependencies,
  ): Promise<UnitSummary> {
