...
const DEFAULT_OPTIONS: Options = {
  ...
  factionAvailabilityViewMode: 'list',
...
function resolveForceBudgetOptimizerLastSkills(
  saved: Partial<Options> | null | undefined,
): ForceBudgetOptimizerLastSkills {
...
function resolveCBTOptionalRules(saved: Partial<Options> | null | undefined): CBTOptionalRules {
  ...
  return {
    quirks: resolveSavedValue(saved?.CBTOptionalRules?.quirks, defaults.quirks),
...
function resolveCBTAutomationOptions(saved: LegacyOptions | null | undefined): CBTAutomationOptions {
  ...
  const legacyHeatMode: AutomationMode | undefined =
    saved?.cbtAutomations === undefined ? undefined : saved.cbtAutomations ? 'yes' : 'no';
  return Object.fromEntries(
    CBT_AUTOMATION_KEYS.map((key) => [
      key,
      resolveSavedValue(
        saved?.cbtAutomationOptions?.[key],
        key === 'heatAndDissipationResolution' && legacyHeatMode !== undefined ? legacyHeatMode : defaults[key],
...
export class OptionsService {
  ...
  public options = signal<Options>({
    ...
    recordSheetPipLayout: DEFAULT_OPTIONS.recordSheetPipLayout,
    ...
    factionAvailabilityViewMode: DEFAULT_OPTIONS.factionAvailabilityViewMode,
  ...
  async initOptions() {
    ...
    this.options.set({
      ...
      megaMekAvailabilityFiltersUseAllScopedOptions:
        saved?.megaMekAvailabilityFiltersUseAllScopedOptions ??
        DEFAULT_OPTIONS.megaMekAvailabilityFiltersUseAllScopedOptions,
      printAllOptions: resolvePrintAllOptions(saved),
      recordSheetDoubleTapZoomReset:
        saved?.recordSheetDoubleTapZoomReset ?? DEFAULT_OPTIONS.recordSheetDoubleTapZoomReset,
      recordSheetPipLayout: resolveSavedValue(
        saved?.recordSheetPipLayout,
        DEFAULT_OPTIONS.recordSheetPipLayout,
        OPTION_VALUES.recordSheetPipLayout,
      ),
      ...
      c3NetworkConnectionsAboveNodes:
        saved?.c3NetworkConnectionsAboveNodes ?? DEFAULT_OPTIONS.c3NetworkConnectionsAboveNodes,
      automaticallyConvertFiltersToSemantic:
        saved?.automaticallyConvertFiltersToSemantic ?? DEFAULT_OPTIONS.automaticallyConvertFiltersToSemantic,
      ...
      factionAvailabilityViewMode: resolveSavedValue(
        saved?.factionAvailabilityViewMode,
        DEFAULT_OPTIONS.factionAvailabilityViewMode,
        OPTION_VALUES.factionAvailabilityViewMode,
      ),
      ...
      enableForceSyncConflictDialog:
        saved?.enableForceSyncConflictDialog ?? DEFAULT_OPTIONS.enableForceSyncConflictDialog,
  ...
  async setPrintOption<K extends keyof PrintAllOptions>(key: K, value: PrintAllOptions[K]): Promise<void> {
    await this.setOption('printAllOptions', { ...this.options().printAllOptions, [key]: value });
  }
  ...
  async updateForceGeneratorOptions(updater: (options: ForceGeneratorOptions) => ForceGeneratorOptions) {
    const updated = {
      ...this.options(),
