...
describe('UnitSearchIndexService', () => {
  ...
  it('measures composite axes before taking percentiles', () => {
    const service = new UnitSearchIndexService();
    const first = createUnit({ armor: 100, internal: 10, run2: 20, jump: 0, as: { Arm: 8, Str: 2 } });
    const second = createUnit({ armor: 10, internal: 100, run2: 0, jump: 20, as: { Arm: 2, Str: 8 } });
  ...
  it('excludes absent measurements but preserves zero and sparse capabilities', () => {
    ...
    const units = Array.from({ length: 100 }, (_, i) =>
      createUnit({
        ...
        heat: null,
        dissipation: null,
        as: { TP: 'BA', TMM: i === 0 ? null : 0, dmg: { dmgL: i === 0 ? '0*' : '0' } },
      }),
    );
  ...
  it('indexes nonzero arc damage with four dropdown inputs and excludes zero-only arcs', () => {
    ...
    for (const token of ['STD', 'CAP', 'SCAP', 'MSL']) {
      ...
      expect(service.getDropdownOptionUniverse('as.specials')).toContain({
        name: token,
        minimumFieldLabels: ['S', 'M', 'L', 'E'],
      });
      expect(
        service
          .getIndexedASSpecials(unit.uuid)
          ?.occurrences.find((occurrence) => occurrence.token === token)
          ?.values.map((value) => value?.rank),
      ).toEqual([1, 2, 3, 4]);
    }
  ...
  it('indexes implicit values and digit-bearing artillery tokens with contextual fields', () => {
    ...
    expect(
      service.getIndexedASSpecials(unit.uuid)?.occurrences.find((occurrence) => occurrence.token === 'SNARC')?.values,
  ...
  it('indexes Alpha Strike zero-star damage between zero and one', () => {
    ...
    expect(service.getUnitStats(createUnit({})).asDmgS).toEqual({
      min: 0.5,
      max: 0.5,
      average: 0.5,
      p95: 0.5,
  ...
  it('tracks min, max, and average stats by subtype and alpha strike type', () => {
    ...
    expect(service.getUnitStats(createUnit({})).weightedMaxRange).toEqual({
      min: 10,
      max: 12,
      average: 11,
      p95: 12,
    ...
    expect(service.getUnitStats(createUnit({ as: { TP: 'WS' } })).dropshipCapacity).toEqual({
      min: 2,
      max: 6,
      average: 4,
      p95: 6,
    ...
    expect(service.getUnitStats(createUnit({ as: { TP: 'WS' } })).gravDecks).toEqual({
      min: 1,
      max: 3,
      average: 2,
      p95: 3,
  ...
  it('returns zeroed min, max, and average values for missing buckets', () => {
    ...
    expect(service.getUnitStats(createUnit({ as: { TP: 'XX' } })).armor).toEqual({
      min: 0,
      max: 0,
      average: 0,
      p95: 0,
    ...
    expect(service.getUnitStats(createUnit({ as: { TP: 'XX' } })).weightedMaxRange).toEqual({
      min: 0,
      max: 0,
      average: 0,
      p95: 0,
    ...
    expect(service.getUnitStats(createUnit({ as: { TP: 'XX' } })).asTmm).toEqual({
      min: 0,
      max: 0,
      average: 0,
      p95: 0,
    ...
    expect(service.getUnitStats(createUnit({ as: { TP: 'XX' } })).gravDecks).toEqual({
      min: 0,
      max: 0,
      average: 0,
      p95: 0,
  ...
  it('includes component indexes in the atomically prepared catalog', () => {
    ...
    const unit = createUnit({
      name: 'Deferred Unit',
      comp: [{ id: 'test-mg', q: 2, n: 'Test MG', t: 'B', p: 1, l: 'RA', eq: antiInfantryWeapon }],
    });
