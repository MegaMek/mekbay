...
describe('CustomUnitsService native construction persistence', () => {
          ...
          useValue: {
            ...
            updateCustomUnits: async (changes: Parameters<DbService['updateCustomUnits']>[0]) => {
              ...
              for (const { uuid, accountUuid, update } of changes) {
                const current = (rows as SavedCustomUnit[]).find(
                  (row) => row.uuid === uuid && (row.accountUuid ?? '') === accountUuid,
                );
                ...
                if (next !== current) {
                  if (next) await save(next);
                    ...
                    cachedRows = cachedRows.filter(
                      (row) =>
                        (row as StoredCustomUnitSummary).uuid !== uuid ||
                        (row as StoredCustomUnitSummary).accountUuid !== accountUuid,
                    );
                  }
                }
                results.push(next);
  ...
  beforeEach(() => {
    rows = [];
    cachedRows = [];
    saveSummaries = jasmine
      .createSpy('saveCustomUnitSummaries')
      .and.callFake(async (summaries: readonly StoredCustomUnitSummary[]) => {
        for (const summary of summaries) {
          cachedRows = [
            structuredClone(summary),
            ...cachedRows.filter(
              (row) =>
                (row as StoredCustomUnitSummary).uuid !== summary.uuid ||
    ...
    save = jasmine.createSpy('saveCustomUnit').and.callFake(async (record: SavedCustomUnit) => {
      rows = [
        ...
        ...rows.filter(
          (row) =>
            (row as SavedCustomUnit).uuid !== record.uuid ||
            ((row as SavedCustomUnit).accountUuid ?? '') !== (record.accountUuid ?? ''),
        ),
      ];
    });
  ...
  it('persists the native refit source and keeps it removed when an existing refit is unlinked', async () => {
    const service = createService();
    const saved = await service.save(mek(), { originalUnitUuid: ORIGINAL });
    expect(saved.source).toContain(`refitfromuuid:${ORIGINAL}`);
    const loaded = await service.load(saved.uuid);
    expect(loaded.refitFromUUID()).toBe(ORIGINAL);
    loaded.refitFromUUID.set(undefined);
    const unlinked = await service.save(loaded, { uuid: saved.uuid, originalUnitUuid: ORIGINAL });
    expect(unlinked.source).not.toContain('refitfromuuid:');
    expect(unlinked.originalUnitUuid).toBeUndefined();
    expect((await service.load(saved.uuid)).refitFromUUID()).toBeUndefined();
  });

  it('preserves a refit reference imported inside a native source without save metadata', async () => {
    const service = createService();
    const entity = mek();
    entity.refitFromUUID.set(ORIGINAL);
    const imported = service.parseDraft(encodeNativeEntity(entity), 'mtf');
    const saved = await service.save(imported);
    expect(saved.originalUnitUuid).toBe(ORIGINAL);
    expect((await service.load(saved.uuid)).refitFromUUID()).toBe(ORIGINAL);
  });
  ...
  it('reuses owned and subscribed summaries after a fresh service loads IndexedDB', async () => {
    ...
    await first.acceptRemote({
      ...owned,
      uuid: ORIGINAL,
      source: encodeNativeEntity(subscribed),
      hash: undefined,
      owned: false,
      subscribed: true,
      pending: undefined,
    ...
    expect((await reloaded.prepareSummaries(DEPENDENCIES)).map((unit) => unit.model).sort()).toEqual(
      expected.map((unit) => unit.model).sort(),
    );
  ...
  it('rebuilds only the changed source after restart, then invalidates persisted dependency revisions', async () => {
    ...
    rows = rows.map((row) =>
      (row as SavedCustomUnit).uuid === saved.uuid ? { ...saved, source: encodeNativeEntity(changed) } : row,
    );
    const projector = await DEPENDENCIES.getProjector();
    const project = spyOn(projector, 'project').and.callThrough();
    spyOn(DEPENDENCIES, 'getProjector').and.resolveTo(projector);
  ...
  it('keeps temporary previews out of the persisted summary cache', async () => {
    ...
    await first.acceptRemote(
      { ...saved, pending: undefined, hash: undefined, source: encodeNativeEntity(preview) },
      false,
    );
  ...
  it('reuses unchanged projections and the summary already checked when saving an edit', async () => {
    ...
    expect(new Map(current.map((unit) => [unit.uuid, unit.mul1id]))).toEqual(
      new Map(initial.map((unit) => [unit.uuid, unit.mul1id])),
    );
    ...
    expect(withAddition.every((unit) => unit.mul1id === null)).toBeTrue();
    expect(withAddition.filter((unit) => unit.model !== 'Third').map((unit) => unit.mul1id)).toEqual(
      current.map((unit) => unit.mul1id),
    );
  });
  ...
  it('rebuilds all obsolete summaries and all summaries affected by dependency changes', async () => {
    ...
    project.calls.reset();
    expect(
      (await service.prepareSummaries(DEPENDENCIES)).every((unit) => unit.summaryVersion === UNIT_SUMMARY_VERSION),
    ).toBeTrue();
    expect(project).toHaveBeenCalledTimes(2);
  ...
  it('copies core MTF into a new UUID without mutating the source and preserves original lineage on update', async () => {
    const service = createService();
    const core = mek({ uuid: ORIGINAL, chassis: 'Crab', model: 'CRB-20', mul1id: 123 });
    core.mulId.set(123);
    ...
    const relabeled = await service.save(editing, {
      uuid: saved.uuid,
      originalUnitUuid: asUnitUuid('019f6767-0dcb-7bb8-992f-000000000002'),
    });
    ...
    expect(summaries[0].mul1id).toBe(123);
  ...
  it('preserves newer edits and deletion intent when an earlier upload completes', async () => {
    ...
    expect(await service.get(first.uuid)).toEqual(
      jasmine.objectContaining({ source: second.source, pending: 'save', syncedHash: first.hash }),
    );
    ...
    expect(service.records()).toEqual([]);
    expect(service.pendingRecords()[0]).toEqual(
      jasmine.objectContaining({ pending: 'delete', syncedHash: second.hash }),
    );
  });
  ...
  it("isolates the same shared design between accounts without losing the owner's pending edits", async () => {
    ...
    await expectAsync(service.save(await service.load(saved.uuid), { uuid: saved.uuid })).toBeRejectedWithError(
      /Clone it/,
    );
  ...
  for (const reverse of [false, true]) {
    it(`hydrates duplicate cache keys using the owned record and its matching source (${reverse})`, async () => {
      const owned: SavedCustomUnit = {
        schemaVersion: 1,
        uuid: ORIGINAL,
        createdAt: 1,
        updatedAt: 1,
        format: 'mtf',
        source: encodeNativeEntity(mek({ uuid: ORIGINAL, model: 'Owned' })),
        accountUuid: 'viewer',
        owned: true,
      };
      const subscribed: SavedCustomUnit = {
        ...owned,
        owned: false,
        updatedAt: 2,
  ...
  it('limits new owned designs to 5,000 while allowing edits, deletions, subscriptions and other accounts', async () => {
    ...
    rows = Array.from({ length: MAX_OWNED_CUSTOM_UNITS }, (_, i) => ({
      schemaVersion: 1,
      uuid: id(i + 1),
      createdAt: 1,
      updatedAt: 1,
  ...
  it('extracts both native image formats under the final UUID before persisting or hashing a design', async () => {
    ...
    for (const entity of [mek(), createTestTankEntity()]) {
      entity.fluffImageEncoded.set(encoded);
      entity.iconEncoded.set(encoded);
      ...
      expect(loaded.fluffImageEncoded()).toBe('');
      expect(loaded.iconEncoded()).toBe('');
  ...
  it('saves and reloads an explicit tileset path and clears it back to automatic', async () => {
    const service = createService();
    const entity = mek();
    entity.iconPath.set('meks/Atlas.png');
    const saved = await service.save(entity);
    const loaded = await service.load(saved.uuid);
    expect(loaded.iconPath()).toBe('meks/Atlas.png');
    expect((await service.prepareSummaries(DEPENDENCIES)).find(unit => unit.uuid === saved.uuid)?.icon).toBe('meks/Atlas.png');
    loaded.iconPath.set('');
    const automatic = await service.save(loaded, { uuid: saved.uuid });
    expect(automatic.source).not.toContain('iconpath');
    expect((await service.load(saved.uuid)).iconPath()).toBe('');
    expect((await service.prepareSummaries(DEPENDENCIES)).find(unit => unit.uuid === saved.uuid)?.icon).toBe('');
  });
});
