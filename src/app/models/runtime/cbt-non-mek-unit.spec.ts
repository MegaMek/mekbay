...
import { TestAeroSpaceFighterEntity,TestProtoMekEntity,TestTankEntity } from '../entity/testing/test-entities';
import { createTestEquipmentRegistry } from '../entity/testing/test-equipment-registry';
import { createNonMekUnit,redeployNonMekCrew,repairNonMekUnit,restoreNonMekUnit } from './cbt-non-mek-unit';
...
describe('CBTNonMekUnit', () => {
    ...
    it('preserves explicit vacancies through restore, repair and redeployment for each crewed family', () => {
        for (const entity of [new TestTankEntity(), new TestProtoMekEntity(), new TestAeroSpaceFighterEntity()]) {
            ...
            const restored = restoreNonMekUnit(unit.serialize(), entity, uuid, scenario);
            for (const ready of [restored, repairNonMekUnit(restored), redeployNonMekCrew(restored, unit.getCrewAssignment())]) {
                const runtime = ready;
                ...
                expect(runtime.dispatch({ type: 'set-spotting', spotting: true }).changed).toBeFalse();
