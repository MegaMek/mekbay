...
export function getInfantryTonnage(entity: InfantryEntity): number {
  return getInfantryWeightBreakdown(entity).rounded;
}

export function getInfantryWeightBreakdown(entity: InfantryEntity) {
  const activeTroopers = Math.max(0, entity.squadSize() * entity.squadCount());
  const mount = entity.mount();
  let personnelAndTransport: number;
  let specialistGear = 0;
  let fieldGunsAndAmmo = 0;
  let exact: number;

  if (mount) {
    const troopsPerCreature = mount.size === 'Large' ? 1 : mount.size === 'Very Large' ? 2 : 4;
    personnelAndTransport = troopsPerCreature > 1
      ? (mount.weight + 0.2 * entity.squadSize()) * entity.squadCount()
      : (mount.weight + 0.2) * activeTroopers;
    exact = personnelAndTransport;
  } else {
    personnelAndTransport = activeTroopers * getTrooperWeightMultiplier(entity);
    ...
    exact = activeTroopers * multiplier;
    specialistGear = exact - personnelAndTransport;
    for (const mounted of entity.equipment()) {
      if ((mounted.location === 'Field Guns' && mounted.equipment instanceof WeaponEquipment)
        ...
        const tonnage = mounted.getTonnage(entity) ?? 0;
        fieldGunsAndAmmo += tonnage;
        exact += tonnage;
      }
    }
  }

  return { personnelAndTransport, specialistGear, fieldGunsAndAmmo, exact, rounded: Math.ceil(exact * 2) / 2 };
}
