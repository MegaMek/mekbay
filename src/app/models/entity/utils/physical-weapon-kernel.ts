...
export function physicalEquipmentKindFromFlags(
  ...
  if (flags.has('F_CLUB')) {
    if (flags.has('S_CLAW')) return 'improvised-claw';
    if (flags.has('S_HATCHET')) return 'hatchet';
    if (flags.has('S_SWORD')) return 'sword';
    if (flags.has('S_LANCE')) return 'lance';
    if (flags.has('S_MACE')) return 'mace';
    if (flags.has('S_RETRACTABLE_BLADE')) return 'retractable-blade';
    if (flags.has('S_VIBRO_LARGE')) return 'vibroblade-large';
    if (flags.has('S_VIBRO_MEDIUM')) return 'vibroblade-medium';
    if (flags.has('S_VIBRO_SMALL')) return 'vibroblade-small';
    if (flags.has('S_PILE_DRIVER')) return 'pile-driver';
    if (flags.has('S_FLAIL')) return 'flail';
    if (flags.has('S_DUAL_SAW')) return 'dual-saw';
    if (flags.has('S_CHAINSAW')) return 'chainsaw';
    if (flags.has('S_BUZZSAW')) return 'buzzsaw';
    if (flags.has('S_BACKHOE')) return 'backhoe';
    if (flags.has('S_MINING_DRILL')) return 'mining-drill';
    if (flags.has('S_WRECKING_BALL')) return 'wrecking-ball';
    if (flags.has('S_CHAIN_WHIP')) return 'chain-whip';
    if (flags.has('S_COMBINE')) return 'combine';
    if (flags.has('S_ROCK_CUTTER')) return 'rock-cutter';
    if (flags.has('S_SPOT_WELDER')) return 'spot-welder';
    return 'club';
  }
