...
import type { MountedEngine } from '../components';
...
/** Master Engine Table mass of the installed engine, or of a candidate mounted in its place. */
export function getEquipmentEngineWeight(entity: BaseEntity, engine: MountedEngine = entity.mountedEngine()): number {
  if (entity.isSupportVehicle()) {
    return getSupportVehicleEngineWeight(entity, engine);
  }

  const usesTankEngine = entity.entityType === 'Tank' || entity.entityType === 'Naval' || entity.entityType === 'VTOL';
  let weight = engine.getWeight({ tank: usesTankEngine });
...
function getSupportVehicleEngineWeight(entity: BaseEntity & SupportVehicle, engine: MountedEngine): number {
  ...
  let weight = getBaseEngineValue(entity) * (4 + movementPoints * movementPoints) * multiplier * entity.tonnage();
  ...
  return entity.weightClass() === 'Small Support' ? Math.round(weight * 1000) / 1000 : roundToNearestHalfTon(weight);
}
...
function getBaseEngineValue(entity: BaseEntity & SupportVehicle): number {
  ...
  switch (entity.motiveType()) {
    case 'Airship':
      return tonnage < 5 ? 0.005 : 0.008;
    case 'Hover':
      return tonnage < 5 ? 0.0025 : isLarge ? 0.008 : 0.004;
    case 'Naval':
    case 'Hydrofoil':
    case 'Submarine':
      return tonnage < 5 ? 0.004 : 0.007;
    case 'Tracked':
      return tonnage < 5 ? 0.006 : isLarge ? 0.025 : 0.013;
    case 'Wheeled':
      return tonnage < 5 ? 0.0025 : isLarge ? 0.015 : 0.0075;
    case 'WiGE':
      return tonnage < 5 ? 0.003 : isLarge ? 0.006 : 0.005;
    case 'Rail':
    case 'MagLev':
      return tonnage < 5 ? 0.003 : isLarge ? 0.005 : 0.004;
    default:
      return 0;
  }
}
