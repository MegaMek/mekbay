...
const OVERFLOW_OVERLAY_KEY = 'panel-tab-overflow-menu';

/**
 * Base panel component providing common structure for panels.
 * Similar to base-dialog but without the modal/overlay wrapper.
 */
@Component({
    selector: 'base-panel',
                        ...
                        <div class="tab-buttons" #tabButtonsContainer [class.wrap]="!overflowToDropdown()">
                            @if (overflowToDropdown()) {
                                ...
                                @if (overflowTabs().length > 0) {
                                    <button type="button"
                                            class="tab-button tab-overflow-trigger"
                                            #overflowTrigger
...
export class BasePanelComponent implements AfterViewInit {
