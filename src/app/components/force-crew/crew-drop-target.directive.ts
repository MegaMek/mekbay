...
export class CrewDropTargetDirective {
    ...
    private readonly canReceive = computed(() => this.force().canEditPersonnel()
        && (this.unitId() === undefined
            ? this.workspace.loadedForces().length > 1
            : this.crew.firstVacantPosition(this.force(), this.unitId()!) !== undefined));
    ...
    constructor() {
        ...
        effect(onCleanup => {
            if (this.canReceive()) onCleanup(this.crew.registerDropList(this.force(), this.list.id));
        });
        effect(() => {
            const canReceive = this.canReceive();
            this.list.disabled = !canReceive;
            this.list.connectedTo = canReceive ? [...this.crew.connectedDropLists()] : [];
        });
