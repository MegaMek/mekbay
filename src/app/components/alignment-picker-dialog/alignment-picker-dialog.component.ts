...
export interface AlignmentPickerResult {
...
@Component({
    ...
    <div class="content">
        ...
        @if (!force) {
            <p>No force data provided. Please try again.</p>
        } @else {
            <force-preview [force]="force"></force-preview>

            @if (force.readOnly()) {
                <p class="muted">This force is read-only, you can load it as a clone to make it editable.</p>
                <div class="clone-option">
                    <label class="agree">
                        <input
                            ...
                            class="bt-checkbox"
                            [checked]="clone()"
                            (change)="clone.set(!clone())"
                        />
                        <span>Load as a clone (editable)</span>
                    </label>
                </div>
            } @else {
                <p class="muted">You own this force, you will be able to edit it.</p>
            }
...
export class AlignmentPickerDialogComponent {
    private dialogRef = inject(DialogRef<AlignmentPickerResult | null>);
    ...
    pick(alignment: ForceAlignment) {
        if (alignment === 'enemy' && this.lobbyService.hasLobby()) return;
        this.dialogRef.close({ alignment, clone: this.clone() });
    }
}
