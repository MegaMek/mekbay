...
import {
  ...
  type UnitDetailsDialogData,
} from '../unit-details-dialog/unit-details-dialog.component';
...
import { isCBTForceMember, type CBTForceMember, type ForceMember } from '../../models/force-member.model';
...
export class ForceBuilderViewerComponent {
  ...
  forceEraWarning(force: Force): string | null {
    ...
    const availabilityContext = this.unitAvailabilitySource.createForceAvailabilityContextForUnits(summaries, eras);
  ...
  toggleForceCollapsed(event: MouseEvent, force: Force) {
    ...
    this.collapsedGroups.update((set) => {
      ...
      for (const g of groups) {
        if (allCollapsed) next.delete(g.id);
        else next.add(g.id);
      }
      return next;
    });
  ...
  constructor() {
    ...
    this.remoteUpdateSub = this.remoteSync.remoteForceUpdated$.subscribe(({ force, alignment }) => {
      ...
      if (isVisible) {
        ...
        // Add to blinking set
        this.blinkingForces.update((set) => {
          const next = new Set(set);
          next.add(force);
          return next;
        });
  ...
  async removeUnit(event: MouseEvent, unit: ForceMember) {
    event.stopPropagation();
    await this.forceCommands.removeUnit(unit, event.ctrlKey || event.metaKey);
  ...
  async openC3Network(event: MouseEvent, unit: ForceMember) {
    event.stopPropagation();
    await this.forceDialogs.openC3Network(unit.force, isCBTForceMember(unit) ? unit.force.readOnly() : unit.readOnly());
  }
  ...
  async drop(event: CdkDragDrop<ForceMember[]>) {
    const groupIdFromContainer = (id?: string) =>
      id && id.startsWith('group-') ? id.substring('group-'.length) : null;
    ...
    if (isCBTForceMember(movingMember)) {
      ...
      if (!(toForce instanceof CBTForce)) {
        this.toastService.showToast('Converting a live CBT unit to Alpha Strike is not supported yet.', 'error');
        return;
      }
    ...
    if (fromForce === toForce) {
      ...
      const preparedConversion = crossSystem
        ? await this.prepareCrossSystemConversion(sourceUnitToConvert, fromForce, toForce)
        : null;
      ...
      this.forceWorkspace.selectUnit(toForce.members().find((member) => member.id === unitToInsert.id) ?? null);
  ...
  private async confirmEmptyForceAfterTransfer(source: CBTForce): Promise<boolean> {
    ...
    return (
      (await this.dialogsService.choose(
        'Remove Empty Force',
  ...
  async dropForNewGroup(event: CdkDragDrop<ForceMember[], ForceMember[], unknown>) {
    ...
    const targetSlot = this.forceWorkspace
      .loadedForces()
      .find((s) => (s.force.instanceId() || s.force.name) === forceKey);
    ...
    if (isCBTForceMember(movingMember)) {
      ...
      if (
        sourceForce !== targetForce &&
        (sourceForce.readOnly() || !(await this.confirmEmptyForceAfterTransfer(sourceForce)))
      )
        return;
    ...
    const preparedConversion = crossSystem
      ? await this.prepareCrossSystemConversion(sourceUnitToConvert, sourceForce, targetForce)
      : null;
    if (crossSystem && !preparedConversion) return;
    ...
    this.forceWorkspace.selectUnit(targetForce.members().find((member) => member.id === unitToInsert.id) ?? null);
  ...
  private async prepareCrossSystemConversion(
    ...
    try {
      ...
      this.toastService.showToast(error instanceof Error ? error.message : 'Could not convert unit.', 'error');
      return null;
    }
  }
  ...
  getFormationMismatchTitle(group: UnitGroup): string {
    ...
    if (formation.requirements) {
      ...
      if (req)
        parts.push(this.buildFormationRequirementTooltipLine(showParentRequirements ? formation.name : null, req));
    }
  ...
  async dropGroup(event: CdkDragDrop<UnitGroup[]>) {
    ...
    if (fromForce === toForce) {
      ...
      if (fromForce instanceof CBTForce || toForce instanceof CBTForce) {
        this.toastService.showToast('Moving a live CBT group between forces is not supported yet.', 'error');
        return;
      }
      ...
      const unitsStillCurrent =
        groupToMove?.units().length === sourceUnits.length &&
      ...
      try {
        toForce.adoptGroup(groupToMove, event.currentIndex, crossSystem ? preparedConversions! : undefined);
      } catch {
        return;
      }
      ...
      if (firstUnit) {
        this.forceWorkspace.selectUnit(toForce.members().find((member) => member.id === firstUnit.id) ?? null);
      }
