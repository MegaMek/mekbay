...
import { MAX_CREW_WOUNDS, type CrewMemberDetails } from '../../models/crew-member.model';
...
describe('CBT multi-crew pilot dialog logic', () => {
  ...
  it('includes additional non-editable skills and falls back for empty crew', () => {
    ...
    expect(getSyntheticCrewSkill([{ ...CREW[0], aeroGunnery: 2 }], 'gunnery')).toBe(2);
  ...
  it('shows no Piloting BV variation at or above another member minimum', () => {
    const entries = buildCrewSkillPreviewEntries(CREW, 1, 'piloting', (gunnery, piloting) => gunnery * 100 + piloting);
  ...
  it('shows no Gunnery BV variation at or above another member minimum', () => {
    const entries = buildCrewSkillPreviewEntries(CREW, 2, 'gunnery', (gunnery, piloting) => gunnery * 100 + piloting);
  ...
  it('previews aerospace skill changes against all ground and crew minima', () => {
    const crew = [
      { id: 0, name: 'LAM Pilot', gunnery: 4, piloting: 5, aeroGunnery: 6, aeroPiloting: 7 },
      { id: 1, name: 'LAM Gunner', gunnery: 3, piloting: 4, aeroGunnery: 5, aeroPiloting: 6 },
    ];
  ...
  it('uses generic and station-specific crew name labels', () => {
    ...
    expect(EditPilotDialogComponent.prototype.crewNameLabel.call({ crew: [...CREW, { ...CREW[0], id: 3 }] }, 3)).toBe(
      'Crew Member 4 Name',
    );
  });
  ...
  it('generates a name for only the requested crew member and prevents duplicate requests', async () => {
    ...
    const harness = {
      ...
      nameInputs: () => [{ nativeElement: firstInput }, { nativeElement: secondInput }],
...
describe('Pilot dialog skill previews and reserve controls', () => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        { provide: LoggerService, useValue: { warn: jasmine.createSpy('warn') } },
        {
          ...
          useValue: { isPhone: signal(false), windowWidth: signal(1024), windowHeight: signal(768) },
        },
  ...
  it('edits zero through six wounds locally and clears the selected wound', async () => {
    data.editWounds = true;
    data.crew = [{ ...data.crew[0], wounds: 2 }];
    const fixture = TestBed.createComponent(EditPilotDialogComponent);
    fixture.detectChanges();
    const segments: NodeListOf<HTMLButtonElement> = fixture.nativeElement.querySelectorAll('.wounds-segment');
    expect(segments.length).toBe(MAX_CREW_WOUNDS);
    expect(fixture.nativeElement.querySelectorAll('.wounds-segment.filled').length).toBe(2);
    // BMM, MechWarrior damage: the sixth wound is fatal; the track ends at six.
    for (let wounds = MAX_CREW_WOUNDS; wounds >= 1; wounds--) {
      segments[wounds - 1].click();
      fixture.detectChanges();
      expect(fixture.componentInstance.crew[0].wounds()).toBe(wounds);
      expect(fixture.nativeElement.querySelectorAll('.wounds-segment.filled').length).toBe(wounds);
      segments[wounds - 1].click();
      expect(fixture.componentInstance.crew[0].wounds()).toBe(wounds - 1);
      // Select another value before checking the next segment.
      if (wounds > 1) segments[MAX_CREW_WOUNDS - 1].click();
    }
    expect(data.crew[0].wounds).toBe(2);
    await fixture.componentInstance.submit();
    expect(close.calls.mostRecent().args[0].crew[0].wounds).toBe(0);
  });

  it('explains untracked wounds and omits the bar for preview pilots', () => {
    data.editWounds = true;
    data.unitTracksWounds = false;
    const fixture = TestBed.createComponent(EditPilotDialogComponent);
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent).toContain('This unit does not track pilot wounds.');
    fixture.componentInstance.close();
    expect(close).toHaveBeenCalledOnceWith(null);
    fixture.destroy();
    data.editWounds = false;
    const preview = TestBed.createComponent(EditPilotDialogComponent);
    preview.detectChanges();
    expect(preview.nativeElement.querySelector('.wounds-bar')).toBeNull();
  });
  ...
  it('opens the matrix between Gunnery and Piloting and applies a selected pair', async () => {
    ...
    expect(close).toHaveBeenCalledWith(
      jasmine.objectContaining({ crew: [jasmine.objectContaining({ gunnery: 3, piloting: 4 })] }),
    );
  });
  ...
  it('allows a CBT reserve commander toggle and omits unavailable BV values', async () => {
    ...
    fixture.componentInstance.submit();
    expect(close).toHaveBeenCalledWith(
      jasmine.objectContaining({ commander: true, crew: [jasmine.objectContaining({ gunnery: 2, piloting: 3 })] }),
    );
  });
  ...
  it('allows an Alpha Strike reserve commander toggle without a unit or group', async () => {
    ...
    fixture.detectChanges();
    const commander = (fixture.nativeElement as HTMLElement).querySelector<HTMLButtonElement>(
