...
export function resolveDisplayStartIndex(totalUnits: number, visiblePages: number, currentViewStartIndex: number): number {
...
export function resolveDisplayedUnits(
    ...
    const startIndex = resolveDisplayStartIndex(totalUnits, visiblePages, currentViewStartIndex);
...
export function buildForceChangePlan(options: {
    allUnits: readonly PageViewerMember[];
    ...
    const totalUnits = allUnits.length;
    let nextViewStartIndex = resolveDisplayStartIndex(totalUnits, visibleCount, currentViewStartIndex);
    ...
    const targetDisplayCount = resolveDisplayedUnits(allUnits, visibleCount, nextViewStartIndex).units.length;
