...
export function resolveSwipeVisibleOffsets(options: {
...
export function buildSwipeInitialRangePlan(options: {
    totalUnits: number;
    ...
    for (let offset = leftmostOffset; offset <= rightmostOffset; offset++) {
        indicesToPrepare.add(normalizeIndex(baseDisplayStartIndex + offset, totalUnits));
    }
...
export function resolveSwipeVisibleOffsetRefresh(options: {
...
export function buildSwipeSlotExtensionPlan(options: {
    totalUnits: number;
    effectiveVisible: number;
    baseDisplayStartIndex: number;
    ...
    while (neededLeftOffset < nextLeftmostOffset && (nextRightmostOffset - nextLeftmostOffset) < maxRange) {
        ...
        const unitIndex = normalizeIndex(baseDisplayStartIndex + newOffset, totalUnits);
...
export function resolveSwipeSlotVisibilityPlan(options: {
...
function normalizeIndex(index: number, totalUnits: number): number {
    if (totalUnits <= 0) {
        return 0;
    }
