...
import { buildSwipeRenderDecision, type PageViewerSwipeRenderDecision } from './page-viewer-swipe-render-plan';
import { resolveSwipeSlotVisibilityPlan } from './page-viewer-swipe-slot';
...
export function buildSwipeRenderUpdate(options: {
    ...
    const bindingPlan = buildSwipeBindingPlan({
    ...
    const slotInstructions = bindingPlan.slotsToProcess.flatMap((slotPlan) => {
        ...
        const decision = buildSwipeRenderDecision({
            addOnly: options.addOnly,
            visiblePages: options.visiblePages,
