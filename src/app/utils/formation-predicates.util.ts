...
function isSupportAircraft(facts: FormationUnitFacts): boolean {
    return facts.unitSubtype.includes('Fixed Wing Support Vehicle');
}
    ...
    return isClassicFighter(facts)
        ...
        || (facts.unitType === 'Aero'
            && (facts.unitSubtype.includes('Small Craft')
                || facts.unitSubtype.includes('DropShip')));
}
...
export const FORMATION_PREDICATES: Readonly<Record<FormationPredicateId, FormationPredicate>> = {
    ...
    'assault-damage': (facts, gameSystem) => asOrCbt(gameSystem, facts.asMediumDamage >= 3, formationCanDealDamage(facts, 25, 7)),
    ...
    'long-damage-2': (facts, gameSystem) => asOrCbt(gameSystem, facts.asLongDamage >= 2, formationCanDealDamage(facts, 10, 18)),
    'long-damage-positive': (facts, gameSystem) => asOrCbt(gameSystem, facts.asLongDamage > 0, formationCanDealDamage(facts, 5, 18)),
    'long-damage-strong': (facts, gameSystem) => asOrCbt(gameSystem, facts.asLongDamage > 1, formationCanDealDamage(facts, 5, 18)),
    'low-medium-damage': (facts, gameSystem) => asOrCbt(gameSystem, facts.asMediumDamage < 2, !formationCanDealDamage(facts, 11, 9)),
    'medium-damage-2': (facts, gameSystem) => asOrCbt(gameSystem, facts.asMediumDamage >= 2, formationCanDealDamage(facts, 10, 9)),
    'medium-damage-positive': (facts, gameSystem) => asOrCbt(gameSystem, facts.asMediumDamage > 1, formationCanDealDamage(facts, 5, 15)),
    ...
    'vtol-unit': (facts) => facts.unitType === 'VTOL',
};
