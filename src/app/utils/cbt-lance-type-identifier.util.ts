    ...
    private static readonly definitions: FormationTypeDefinition[] = [
        ...
        // ─── Assault Lance ───────────────────────────────────────────────
        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const heavyOrLarger = units.filter(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) >= 3);
                const hasLight = units.some(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === 0);
                
                if (heavyOrLarger.length < 3 || hasLight) return false;
                
                const hasEnoughArmor = units.every(u => u.getUnit().armor >= 135);
                const highDamage = units.filter(u => CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 25, 7));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const allMediumOrLarger = units.every(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) >= 1);
                const hasEnoughArmor = units.every(u => u.getUnit().armor >= 105);
                const hasWeapons = units.filter(u => CBTLanceTypeIdentifierUtil.hasAutocannon(u.getUnit()) || 
                    CBTLanceTypeIdentifierUtil.hasLRM(u.getUnit()) || CBTLanceTypeIdentifierUtil.hasSRM(u.getUnit()));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const lightUnits = units.filter(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === 0);
                const hasAssault = units.some(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === 4);
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const mediumUnits = units.filter(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === 1);
                const hasAssault = units.some(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === 4);
                if (isOnlyCombatVehicles(units)) {
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const mediumOrHeavy = units.filter(u => {
                    const weight = CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit());
                    return weight === 1 || weight === 3;
                });
                
                const withAutocannon = units.filter(u => CBTLanceTypeIdentifierUtil.hasAutocannon(u.getUnit()));
...

        {
            ...
            validator: (units: ForceUnit[]) => {                
                ...
                const firstWeight = CBTLanceTypeIdentifierUtil.getWeightClass(units[0].getUnit());
                const sameWeight = units.every(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === firstWeight);
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const hasAntiAir = units.filter(u => CBTLanceTypeIdentifierUtil.hasLBXAutocannon(u.getUnit()) || 
                    CBTLanceTypeIdentifierUtil.hasAutocannon(u.getUnit()));
                const hasArtillery = units.filter(u => CBTLanceTypeIdentifierUtil.hasArtillery(u.getUnit()));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const longRangeHighDamage = units.every(u => CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 10, 18));
...

        {
            ...
            minUnits: 4,
            validator: (units: ForceUnit[]) => {
                const indirectCapable = units.filter(u => CBTLanceTypeIdentifierUtil.hasLRM(u.getUnit()) || 
                    CBTLanceTypeIdentifierUtil.hasArtillery(u.getUnit()));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const noHeavy = units.every(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) < 3);
        ...
        // ─── Pursuit Lance ───────────────────────────────────────────────
        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const lightOrMedium = units.every(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) <= 1);
                ...
                const hasLongRange = units.some(u => CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 5, 15));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const noAssault = units.every(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) < 4);
                ...
                const hasDamage = units.every(u => CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 10, 9));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const hasDamage = units.every(u => CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 10, 6));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const hasHeavyOrAssault = units.some(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) >= 3);
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const allLight = units.every(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === 0);
        ...
        // ─── Security Lance ─────────────────────────────────────────────
        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const assaultCount = units.filter(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) === 4).length;
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const noLight = units.every(u => CBTLanceTypeIdentifierUtil.getWeightClass(u.getUnit()) > 0);
                // At least one unit in this Formation must have a weapon that can do at least 5 damage at a range of 18 hexes or more.
                const hasLongRange = units.some(u => CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 5, 18));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const lowDamage = units.every(u => !CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 10, 9));
...

        {
            ...
            validator: (units: ForceUnit[]) => {
                ...
                const hasLongRange = units.filter(u => CBTLanceTypeIdentifierUtil.canDealDamage(u.getUnit(), 5, 18));
