...
import type {
  MultiStateOption,
  MultiStateSelection,
} from '../components/multi-select-dropdown/multi-select-dropdown.component';
...
function normalizeUnitRulesRefBuckets(values: unknown): string[][] {
  ...
  return values.map(normalizeRulesRefBucket).filter((bucket) => bucket.length > 0);
}
...
export function normalizeMultiStateSelection(value: unknown): MultiStateSelection {
  ...
  for (const [rawName, rawOption] of Object.entries(value as Record<string, unknown>)) {
    ...
    selection[name] = {
      ...
      count: typeof option.count === 'number' && Number.isFinite(option.count) && option.count > 0 ? option.count : 1,
...
export function isEmbeddedApostrophe(text: string, index: number): boolean {
  return text[index] === "'" && isAlphaNumericChar(text[index - 1]) && isAlphaNumericChar(text[index + 1]);
}
