...
export function buildReferenceTableView(
  ...
  if (optionId === 'vehicle-vtol') {
    ...
    return withUnitClusterTable([locationTable, createVtolCriticalTable()], locationTable, context);
  }
...
export function resolveReferenceTableRoll(
  ...
  const row = table.rows.find(
    (candidate) => candidate.roll && roll >= candidate.roll.min && roll <= candidate.roll.max,
  );
...
function kickCell(value: string, rowIndex: number): ReferenceTableCell {
  return rowIndex % 3 === 0 ? { value, rowSpan: 3 } : { value, continuation: true };
}
...
function combineRollTables(left: ReferenceTableDefinition, right: ReferenceTableDefinition): ReferenceTableDefinition {
  ...
  return {
    ...
    columns: [left, right].flatMap((sourceTable) =>
  ...
  return [
    ...
    compactTable(
      infoTable(
        ...
        [
          { key: 'equipment', label: 'BATTLE ARMOR EQUIPMENT' },
          { key: 'modifier', label: 'MODIFIER' },
        ],
  ...
  return [
    compactTable(
      infoTable(
        ...
        [
          ...
          { key: 'damage', label: 'DAMAGE VS. CONVENTIONAL INFANTRY' },
        ],
    ...
    infoTable(
      ...
      [
        ...
        { key: 'troopers', label: 'CONVENTIONAL TROOPERS HIT†' },
      ],
    ...
    rows: rows.map((source, rowIndex): ReferenceTableRow => {
      ...
      return {
        ...
        cells: Object.fromEntries(
          columnKeys.map((columnKey, columnIndex) => [columnKey, String(source[columnIndex + 1])]),
        ),
      };
    }),
    notes,
...
function infoTable(
  ...
  return {
    ...
    rows: rows.map((source, rowIndex): ReferenceTableRow => ({
      key: `row-${rowIndex}`,
      cells: Object.fromEntries(columns.map((column, columnIndex) => [column.key, source[columnIndex] ?? ''])),
    })),
    notes,
  };
}
