...
import {
  conventionalInfantryRangeWeapon,
  projectConventionalInfantryCombat,
} from '../../../models/rules/conventional-infantry-combat-rules';
...
export class ConventionalInfantryRecordSheetLayout extends CompactRecordSheetLayout {
  public constructor() {
    super('conventional-infantry', 'infantry', 'CONVENTIONAL INFANTRY RECORD SHEET', (page) =>
      page.format === 'a4' ? { height: 186.5, stride: 180.285 } : { height: 174, stride: 167.411 },
    );
  }
  ...
  protected override drawCompactMastheadIcon(parent: SVGGElement, box: Box, svg: SVGSVGElement): void {
    ...
    setAttributes(use, {
      href: '#mekbay-infantry-masthead-art',
      class: 'infantry-masthead-icon',
      width: (56.7 * box.width) / 31.018,
      height: (45.357 * box.height) / 41.357,
    });
  ...
  public override drawCompactPageSupplement(
    ...
    if (blocks.length === 1) {
      const strength =
        entity !== undefined && isInfantryEntity(entity)
          ? infantryShootingStrength(entity)
          : INFANTRY_STRENGTH_CELL_COUNT;
      drawClusterHitsReference(
        page,
        scalePageBox(profile, {
          x: 18.9,
          y: 255.768,
          width: 576.149,
          height: 180,
  ...
  protected async drawCompact(svg: SVGSVGElement, entity: BaseEntity): Promise<void> {
    ...
    const group = addFrame(svg, entity.displayName(), frameBox, {
      // Leave room for the Armor Type label to the right of the title lip.
      maxHeaderWidth: (270 * frameBox.width) / 576,
      bottomLeftNotchWidth: 296,
      cornerAngleDegrees: { topRight: 45, bottomLeft: 45, bottomRight: 60 },
    });
    ...
    addText(
      group,
      infantryDamageDivisor(entity).toFixed(1) + (entity.effectiveEncumberingArmor() ? 'E' : ''),
      x(543.987),
      y(ARMOR_Y),
      { size: font(8.6), maxWidth: x(26) },
    );
    ...
    const facts: readonly [string, string, number, string?][] = [
      ...
      [
        "Anti-'Mech Skill:",
        entity.canMakeAntiMekAttacks() ? String(effectiveEntityPilotingSkill(entity, 5)) : '—',
        53.332,
        'pilotingSkill0',
      ],
    ...
    drawInfantryNotes(
      group,
      infantryNotes(entity),
      { x: x(3.46), y: y(100.441), width: x(108), height: y(64) },
      fontScale,
      sy,
    );
    ...
    const bv = addText(
      group,
      formatNumber(adjustEntityBattleValueForSkills(entity, entity.battleValue(), 4, 5)),
      x(130.701),
      y(154.786),
      { size: font(7.2) },
    );
...
function drawInfantryReferenceTables(page: SVGSVGElement, profile: RecordSheetPageProfile): void {
  drawInfantryBurstFireReference(
    page,
    scalePageBox(profile, {
      x: 18.966,
      y: 585,
  ...
  drawInfantryNonInfantryWeaponReference(
    page,
    scalePageBox(profile, {
      x: 309.966,
      y: 593.214,
...
function drawInfantryNotes(
  ...
  lines.forEach((line, index) =>
    addText(group, line, box.x, box.y + index * (size + 1) * verticalScale, { size: size * fontScale }),
  );
}
...
function infantryNotes(entity: InfantryEntity): string {
  ...
  const burst =
    (rangeWeapon?.hasFlag('F_INF_BURST') || (primary?.infantry.damage ?? 0) > 0.6 ? 1 : 0) + (mount?.burstDamage ?? 0);
  ...
  if (
    [primary, secondary].some((weapon) =>
      (['F_PLASMA', 'F_INCENDIARY_NEEDLES', 'F_INFERNO', 'F_FLAMER'] as const).some((flag) => weapon?.hasFlag(flag)),
    )
  )
    notes.push('Flame-based weapon.');
  ...
  const augmentations = entity
    .augmentations()
    .map((id) => PILOT_ABILITIES.find((ability) => ability.id === id)?.name ?? INFANTRY_AUGMENTATION_NAMES[id] ?? id);
...
function infantryMovementRows(entity: InfantryEntity): readonly { id: string; value: number; label: string }[] {
  ...
  if (motive === 'Jump') rows = [{ id: 'mpJump', value: entity.jumpMP(), label: 'Jump' }, ground];
    ...
    rows = [
      {
        id: 'mpJump',
        value: entity.umuMP(),
        label: motive === 'Submarine' ? 'Mechanized SCUBA' : entity.isMotorizedScuba() ? 'SCUBA (Motorized)' : 'SCUBA',
      },
    ];
    ...
    rows = [
      {
        ...ground,
        label: ['Tracked', 'Wheeled', 'Hover'].includes(motive)
          ? `Mechanized ${motive}`
          : motive === 'Motorized'
            ? 'Motorized'
            : 'Ground',
...
function drawCompactInfantryTrack(
  ...
  const track = SvgFrameUtil.createSVGFrame('', width, 114.229, {
    variant: 'nested',
    showHeader: false,
    cornerAngleDegrees: { topLeft: 0, topRight: 0, bottomLeft: 45, bottomRight: 45 },
  });
  track.setAttribute('class', 'infantry-strength-track');
  track.setAttribute(
    'transform',
    `translate(${formatNumber(x(left))} ${formatNumber(y(top))}) ` + `scale(${formatNumber(Math.min(x(1), y(1)))})`,
  );
  ...
  createInfantryStrengthDisplay(svg, displayHost).render(
    combat,
    {
...
function drawInfantryFieldGuns(track: SVGGElement, entity: InfantryEntity): void {
  ...
  const ammo = mounts.reduce(
    (sum, mount) => sum + (mount.equipment instanceof AmmoEquipment ? mount.equipment.shots : 0),
    0,
  );
  const group = svgElement('g');
