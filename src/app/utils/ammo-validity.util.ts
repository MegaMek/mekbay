...
export type AmmoSelectionIssueReason =
  | 'not-yet-existing-in-era'
  | 'extinct-in-era'
...
export class AmmoValidityUtil {
  static isAmmoValid(ammo: AmmoEquipment, context: AmmoValidityContext = {}): boolean {
    return context.unitType !== 'Aero' || this.canAeroUse(ammo, !!context.allowAeroArtilleryAlternateMunitions);
  }
  ...
  static getAmmoSelectionStatus(
    ammoOptions: readonly AmmoEquipment[],
    context: AmmoValidityContext = {},
  ): Record<string, AmmoSelectionStatus> {
    return Object.fromEntries(
      ammoOptions.map((ammo) => [ammo.internalName, { issues: this.getAmmoSelectionIssues(ammo, context) }]),
    );
  }

  static getAmmoSelectionIssues(ammo: AmmoEquipment, context: AmmoValidityContext = {}): AmmoSelectionIssue[] {
    const reasons = [
      ...(this.isAmmoCompatibleWithWeaponTechBases(ammo, context.weaponTechBases)
        ? []
        : ['incompatible-tech-base' as const]),
  ...
  private static getEraSelectionIssueReasons(ammo: AmmoEquipment, era: Era | null): AmmoSelectionIssueReason[] {
    ...
    const timelines = [ammo.tech.advancement?.is, ammo.tech.advancement?.clan].filter(
      (dates): dates is TechAdvancementDates => !!dates,
    );
  ...
  private static getTimelineSelectionIssueReason(
    dates: TechAdvancementDates,
    era: Era,
  ): AmmoSelectionIssueReason | null {
    ...
    const nextAfterExtinction = nonExtinctionYears.filter((year) => year > extinctYear).sort((a, b) => a - b)[0];
