...
import { constructionRuntimeBattleValue, constructionRuntimeSource, prepareConstructionRuntime, type ConstructionRuntimeChanges } from '../models/runtime/construction-runtime';
...
describe('construction component blocks', () => {
    const ac = new WeaponEquipment({ id: 'Test AC', name: 'Test AC', type: 'weapon',
        tech: { base: 'IS', level: 'Introductory', advancement: { is: { common: '2000' } } },
        flags: ['F_AC', 'F_BALLISTIC', 'F_MEK_WEAPON'], stats: { criticalSlots: 3, tonnage: 5, bv: 50 },
        weapon: { ammoType: 'AC', rackSize: 5, damage: 5, heat: 1, ranges: [6, 12, 18, 24] } });
    const sinks = new MiscEquipment({ id: 'ISDoubleHeatSink', name: 'Double Heat Sink', type: 'misc',
        flags: ['F_DOUBLE_HEAT_SINK', 'F_MEK_EQUIPMENT'], stats: { criticalSlots: 3, tonnage: 1 } });
    const srm = new WeaponEquipment({ id: 'SRM 6', name: 'SRM 6', type: 'weapon',
        tech: { base: 'IS', level: 'Introductory', advancement: { is: { common: '2000' } } },
        flags: ['F_MEK_WEAPON', 'F_MISSILE'], stats: { criticalSlots: 2, tonnage: 3 },
        weapon: { ammoType: 'SRM', rackSize: 6, heat: 4 } });
    const ammo = new AmmoEquipment({ id: 'Test AC Ammo', name: 'Test AC Ammo', shortName: 'Test AC', type: 'ammo',
        stats: { criticalSlots: 1, tonnage: 1 }, ammo: { type: 'AC', rackSize: 5, shots: 20 },
        tech: { base: 'IS', level: 'Introductory', advancement: { is: { common: '2000' } } } });
    const splitAc = new WeaponEquipment({ id: 'Test Split AC', name: 'Test Split AC', type: 'weapon',
        tech: { base: 'IS', level: 'Introductory', advancement: { is: { common: '2000' } } },
        flags: ['F_AC', 'F_BALLISTIC', 'F_MEK_WEAPON'], stats: { criticalSlots: 10, tonnage: 14 },
        weapon: { ammoType: 'AC', rackSize: 20, damage: 20, heat: 7, ranges: [3, 6, 9, 12] } });
    const flak = new AmmoEquipment({ id: 'Test Flak AC Ammo', name: 'Flak Test AC Ammo', shortName: 'Flak Test AC', type: 'ammo',
        stats: { criticalSlots: 1, tonnage: 1 }, ammo: { type: 'AC', rackSize: 5, shots: 20, munitionType: ['M_FLAK'] },
        tech: { base: 'IS', level: 'Standard', advancement: { is: { common: '2000' } } } });
    const endo = new StructureEquipment({ id: 'IS Endo Steel', name: 'Endo Steel', type: 'structure', aliases: ['Endo Steel'],
        tech: { base: 'IS', level: 'Standard', advancement: { is: { common: '2000' } } },
        flags: ['F_ENDO_STEEL', 'F_MEK_EQUIPMENT'], stats: { criticalSlots: 14, spreadable: true, tonnage: 'variable' }, structure: { typeId: 1 } });
    const stealth = new ArmorEquipment({ id: 'IS Stealth', name: 'Stealth', type: 'armor', aliases: ['IS Stealth Armor'],
        tech: { base: 'IS', level: 'Standard', advancement: { is: { common: '2000' } } },
        flags: ['F_STEALTH', 'F_MEK_EQUIPMENT'], stats: { criticalSlots: 12, spreadable: true, tonnage: 'variable' }, armor: { type: 'STEALTH' } });
    const registry = createTestEquipmentRegistry({ [ac.id]: ac, [splitAc.id]: splitAc, [sinks.id]: sinks, [ammo.id]: ammo, [flak.id]: flak,
        [endo.id]: endo, [stealth.id]: stealth, [srm.id]: srm });

    async function create(options: { ammo?: boolean; ruleset?: CBTRuleset; gyro?: GyroType; armored?: boolean; custom?: boolean; editDesign?: boolean; originalBuildYear?: number; armor?: boolean; material?: 'endo' | 'stealth'; missingMaterial?: boolean; materialAlias?: boolean } = {}) {
        ...
        entity.addEquipment({ equipmentId: ac.id, equipment: ac,
            allocation: { kind: 'location', location: 'LT', placements: [0, 1, 2].map(slotIndex => ({ location: 'LT', slotIndex })) },
            rearMounted: false, turretMounted: false, omniPodMounted: false, armored: false });
        if (options.ammo) entity.addEquipment({ equipmentId: ammo.id, equipment: ammo,
        ...
        if (options.materialAlias) entity.updateEquipment(mounts => mounts.map(mount => mount.equipmentId === endo.id
            ? mount.clone({ equipmentId: endo.name }) : mount));
        if (options.missingMaterial) {
            for (const mount of entity.equipment().filter(mount => mount.equipmentId === endo.id || mount.equipmentId === stealth.id)) entity.removeEquipment(mount);
        }
        if (options.armor) {
            entity.setArmorValue('LT', 'front', 12);
            entity.setArmorValue('LT', 'rear', 6);
        }
        const instance = await createMekUnit({ uuid: entity.uuid(), instanceId: 'unit:construction-blocks' }, entity, entity.uuid(), {
            initializerRevision: 1, profileId: 'pristine', deployment: { id: 'test' },
            scenario: { id: 'construction-blocks', ruleset: options.ruleset ?? 'core-2026' },
        });
        const force = { readOnly: () => false,
            ...
            previewConstructionBattleValue: (_member: CBTForceMember, draft: BaseEntity, origins: ConstructionMountOrigins, changes?: ConstructionRuntimeChanges) =>
                constructionRuntimeBattleValue(instance, draft, origins, changes?.commands ?? [], { id: 'construction-blocks', ruleset: options.ruleset ?? 'core-2026' }),
            ...
            previewConstructionRuntime: jasmine.createSpy('previewConstructionRuntime').and.callFake(async (_member, changes: ConstructionRuntimeChanges) => {
                const candidate = await prepareConstructionRuntime(instance, changes.commands, { id: 'construction-blocks', ruleset: options.ruleset ?? 'core-2026' });
                return { changes, snapshot: { entity, ruleset: instance.ruleset(), ...candidate.captureRuntime() },
        ...
        TestBed.configureTestingModule({ providers: [
            ...
            { provide: CustomUnitsService, useValue: { get: async () => options.custom ? { uuid: entity.uuid() } : undefined, summaries: signal([]), parseDraft,
            ...
            { provide: DialogsService, useValue: { requestConfirmation: jasmine.createSpy('requestConfirmation').and.resolveTo(false), createDialog: jasmine.createSpy('createDialog') } },
        ...
        const systems = (system: MekSystemType, location = 'CT') => [...root.querySelectorAll<HTMLElement>(`[data-location="${location}"] .system-slot`)]
            .filter(block => block.querySelector('.system-name .equipment-label')?.textContent?.trim() === editor.systemLabel(system));
        const hit = async (location: string, slotIndex: number, target: 'committed' | 'pending' = 'committed') => {
            const slot = [...instance.getIndex().slots.values()].find(slot => slot.slotIndex === slotIndex
                && instance.getIndex().locations.get(slot.locationId)?.code === location)!;
    ...
    async function drag(f: Awaited<ReturnType<typeof create>>, source: HTMLElement, target: HTMLElement) {
        ...
        const x = start.left + Math.min(20, start.width / 2), y = start.top + Math.min(10, start.height / 2);
        const targetX = end.left + end.width / 2, targetY = end.top + end.height / 2;
        expect(document.elementFromPoint(x, y)?.closest('.cdk-drag')).withContext(`Source at ${x},${y}; target at ${targetX},${targetY}`).toBe(source);
        const handle = source.querySelector<HTMLElement>('.equipment-name') ?? source;
        handle.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0, buttons: 1, clientX: x, clientY: y }));
        handle.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, detail: 1, button: 0, buttons: 1, clientX: x, clientY: y }));
        document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: x + 10, clientY: y + 10 }));
        f.view.detectChanges();
        expect(f.editor.dragging()).withContext('Drag started').not.toBeNull();
        document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: targetX, clientY: targetY }));
        f.view.detectChanges();
        const warehouseZone = target.closest('.warehouse')?.querySelector('.warehouse-drop-zone');
        expect(warehouseZone?.classList.contains('cdk-drop-list-dragging')
            || target.closest('.cdk-drop-list')?.classList.contains('cdk-drop-list-dragging')).withContext(
    ...
    it('uninstalls mounted equipment, installs the same component, and removes it through REMOVE', async () => {
        ...
        const gun = f.editor.entity().equipment().find(mount => mount.equipmentId === ac.id)!;
        ...
        expect(f.editor.entity().equipment().some(mount => mount.mountId === gun.mountId)).toBeFalse();
        expect(f.editor.unallocated()).toEqual([]);
    });
    ...
    it('supports moves in both directions through the unallocated drop zone and clicking an empty slot', async () => {
        ...
        f.editor.onDrop({ item: { data: { equipmentId: ac.id, mountId: gun.mountId } },
            container: { data: { location: 'RA' } }, isPointerOverContainer: true });
        expect(f.editor.selectedMount()?.location).toBe('RA');
        expect(f.editor.entity().equipment().filter(mount => mount.equipmentId === ac.id).length).toBe(1);
    });
    ...
    it('opens the empty drop zone and moves equipment in both directions with pointer dragging', async () => {
        ...
        try {
            ...
            expect(zone.getBoundingClientRect().bottom).toBeLessThanOrEqual(f.root.querySelector('.workshop-footer')!.getBoundingClientRect().top);
            ...
            expect(f.editor.unallocated()).toEqual([]);
            expect(f.editor.entity().equipment().find(mount => mount.mountId === gun.mountId)?.location).toBe('RA');
            expect(f.root.querySelector('.unallocated-empty')).not.toBeNull();
        } finally {
    ...
    it('drags warehouse equipment into the unallocated tray and removes both staged and installed equipment through the warehouse', async () => {
        ...
        Object.assign(f.root.querySelector<HTMLElement>('.workshop-body')!.style, {
            display: 'grid', gridTemplateColumns: '290px minmax(0, 1fr)', gridTemplateRows: 'minmax(0, 1fr)',
        ...
        try {
            ...
            const catalogRow = () => [...f.root.querySelectorAll<HTMLElement>('.warehouse-item')]
                .find(row => row.querySelector('.equipment-label')?.textContent?.trim() === ac.name)!;
            ...
            expect(f.editor.entity().equipment().filter(mount => mount.equipmentId === ac.id).length).toBe(2);
            expect(catalogRow()).toBeDefined();
            await drag(f, f.root.querySelector<HTMLElement>('.unallocated-equipment')!, warehouse.querySelector<HTMLElement>('.warehouse-heading')!);
            expect(f.editor.unallocated()).toEqual([]);
            expect(f.editor.entity().equipment().filter(mount => mount.equipmentId === ac.id).length).toBe(1);
            f.editor.undo();
            ...
            expect(f.editor.entity().equipment().some(mount => mount.equipmentId === ac.id)).toBeFalse();
            expect(f.root.querySelector('[data-location="LT"] .installed-equipment')).toBeNull();
            expect(f.editor.selectedMount()).toBeNull();
            f.editor.undo();
            expect(f.editor.entity().equipment().find(mount => mount.equipmentId === ac.id)?.location).toBe('LT');
            expect(TestBed.inject(ToastService).showToast).not.toHaveBeenCalledWith(jasmine.any(String), 'error');
        } finally {
            document.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, button: 0 }));
            f.root.removeAttribute('style');
    ...
    it('stages equipment without requiring space in the selected location and records one undo step', async () => {
        ...
        expect(f.editor.unallocated().map(mount => mount.equipmentId)).toEqual([splitAc.id]);
        const staged = f.editor.unallocated()[0];
        f.editor.onWarehouseDrop({ item: { data: { equipmentId: splitAc.id, mountId: staged.mountId } }, isPointerOverContainer: false });
    ...
    it('keeps optional unallocated inventory in session history but omits it from the saved draft', async () => {
        const f = await create();
        f.editor.uninstall(f.editor.entity().equipment().find(mount => mount.equipmentId === ac.id)!);
        const draft = encodeNativeEntity(f.editor.entity());
        expect(parseEntity(draft, 'draft.mtf', registry).entity.equipment().some(mount => mount.equipmentId === ac.id)).toBeFalse();
        f.editor.undo();
        expect(f.editor.entity().equipment().find(mount => mount.equipmentId === ac.id)?.location).toBe('LT');
        f.editor.redo();
        expect(f.editor.unallocated().map(mount => mount.equipmentId)).toEqual([ac.id]);
    ...
    it('removes all removable unallocated equipment in one undo step while preserving required and installed mounts', async () => {
        ...
        const installedAmmo = f.editor.entity().equipment().find(mount => mount.equipmentId === ammo.id)!;
        ...
        expect(f.editor.entity().equipment().some(mount => stagedIds.includes(mount.mountId))).toBeFalse();
    ...
    for (const material of ['endo', 'stealth'] as const) {
        it(`restores missing ${material} reservations from the chassis and protects them from removal`, async () => {
            ...
            expect(f.editor.unallocated().length).toBe(1);
            expect(f.editor.entity().equipment().filter(mount => mount.equipmentId === equipment.id).length).toBe(1);
            f.editor.undo();
            expect(f.editor.entity().equipment().find(mount => mount.equipmentId === equipment.id)?.placements?.length).toBe(material === 'endo' ? 14 : 12);
            f.editor.redo();
    ...
    it('recognizes material aliases in loaded critical slots without duplicating the required component', async () => {
        ...
        const mount = f.editor.entity().equipment().find(mount => mount.equipmentId === endo.name)!;
        expect(mount).toBeDefined();
        expect(f.editor.unallocated()).toEqual([]);
        expect(f.editor.requiredMount(mount)).toBeTrue();
        ...
        f.editor.onDrop({ item: { data: { equipmentId: endo.name, mountId: mount.mountId } },
            container: { data: { location: 'RT' } }, isPointerOverContainer: true });
        expect(f.editor.entity().equipment().filter(mount => mount.equipment?.id === endo.id).length).toBe(1);
    ...
    it('edits spreadable slots by location, exposes the remainder, and keeps one component identity', async () => {
        ...
        expect([...inspector.querySelectorAll('.mount-controls label')].some(label => label.textContent?.trim().startsWith('Size'))).toBeFalse();
    ...
    it('uninstalls only the clicked spread block, including a fragment separated by system slots', async () => {
        ...
        const block = f.root.querySelector<HTMLElement>(`[data-location="CT"] [data-slot-index="10"] [data-mount-id="${mountId}"]`)!;
    ...
    it('shows prescribed Stealth shares and accepts only the remaining slots that fit a location', async () => {
        ...
        expect([...f.root.querySelectorAll('.spread-location small')].every(label => label.textContent === '2 required')).toBeTrue();
        ...
        expect(f.editor.selectedSpread()?.remaining).toBe(10);
        expect(f.root.querySelector<HTMLButtonElement>('[aria-label="Allocate one slot to Left Torso"]')?.disabled).toBeTrue();
        ...
        expect(f.editor.selectedSpread()?.locations.filter(location => location.limit).every(location => location.count === 2)).toBeTrue();
        expect(TestBed.inject(ToastService).showToast).not.toHaveBeenCalledWith(jasmine.any(String), 'error');
    });
    ...
    it('locks allocation controls on fixed designs while retaining authored size controls for ordinary equipment', async () => {
        ...
        const mount = f.editor.entity().equipment().find(mount => mount.equipment?.id === endo.id)!;
        f.editor.selectMount(mount);
        f.editor.inspector.open(f.root);
        f.view.detectChanges();
        expect([...f.root.querySelectorAll<HTMLInputElement>('.spread-stepper input')].every(input => input.disabled)).toBeTrue();
        expect([...f.root.querySelectorAll<HTMLButtonElement>('.spread-stepper button')].every(button => button.disabled)).toBeTrue();
        const placements = mount.placements;
        f.editor.setSpreadSlots('LT', 0);
        expect(f.editor.selectedMount()?.placements).toEqual(placements);
        const cargo = new MiscEquipment({ id: 'Cargo', name: 'Cargo', type: 'misc', stats: { tonnage: 'variable', criticalSlots: 1 }, flags: ['F_CARGO'] });
    ...
    it('uninstalls a complete split weapon and keeps required materials when stripping equipment', async () => {
        ...
        expect(f.editor.locations().every(location => location.slots.every(slot => slot.mount?.mountId !== split.mountId))).toBeTrue();
        f.editor.stripEquipment();
        expect(f.editor.unallocated().map(mount => mount.equipmentId)).toEqual([endo.id]);
        expect(f.editor.entity().equipment().filter(mount => mount.allocation.kind === 'engine').length).toBe(10);
    });

    it('protects fixed designs and integral equipment from uninstallation', async () => {
        const f = await create({ editDesign: false });
        const gun = f.editor.entity().equipment().find(mount => mount.equipmentId === ac.id)!;
        f.editor.onUnallocatedDrop({ item: { data: { equipmentId: ac.id, mountId: gun.mountId } }, isPointerOverContainer: true });
        f.editor.onUnallocatedDrop({ item: { data: { equipmentId: ac.id } }, isPointerOverContainer: true });
        f.editor.onWarehouseDrop({ item: { data: { equipmentId: ac.id, mountId: gun.mountId } }, isPointerOverContainer: true });
        expect(f.editor.unallocated()).toEqual([]);
        expect(f.editor.entity().equipment().find(mount => mount.mountId === gun.mountId)).toBe(gun);
        expect(f.root.querySelector('[data-location="LT"] .uninstall-mount')).toBeNull();
        f.editor.setDesignEditing(true);
        const integral = f.editor.entity().equipment().find(mount => mount.allocation.kind === 'engine')!;
        ...
        expect(f.editor.entity().equipment().find(mount => mount.mountId === integral.mountId)).toBe(integral);
        expect(f.editor.unallocated()).toEqual([]);
    });
    ...
    it('shows a staged custom ammo name, marker and primary color, then clears them for the original loadout', async () => {
        ...
        for (const [equipment, quantity, name] of [
            ...
            f.view.detectChanges();
            expect(TestBed.inject(ToastService).showToast).not.toHaveBeenCalledWith(jasmine.any(String), 'error');
            expect(dialogs.createDialog).toHaveBeenCalledWith(SetAmmoDialogComponent, jasmine.objectContaining({
            ...
            const sheet = projectMekRecordSheet(source.entity, source.index, source.ruleset, source.state, source.query, emptyCBTEncounterSnapshot(), null);
            const sheetAmmo = sheet.criticalSlots.flatMap(slot => slot.components).find(component => component.ammo)?.ammo;
            expect(sheetAmmo).toEqual(jasmine.objectContaining({
                displayName: equipment === flak ? 'Flak Test AC Ammo' : 'Test AC Ammo',
                custom: equipment === flak, remaining: quantity,
            }));
            const detailsAmmo = buildUnitComponentMetadata(source.entity, source.query).find(component => component.t === 'X')!;
    ...
    for (const custom of [false, true]) {
        it(`renders the correct disabled and edited save actions for a ${custom ? 'custom' : 'core'} force unit`, async () => {
            ...
            f.editor.setField(f.editor.fields().find(field => field.id === 'model')!, 'Locked change');
            expect(f.editor.dirty()).toBeFalse();
            f.editor.setDesignEditing(true);
            f.editor.setField(f.editor.fields().find(field => field.id === 'model')!, 'A changed design');
            f.view.detectChanges();
            await f.view.whenStable();
    ...
    it('keeps optional OEM year beside introduction year and aligns chassis checkboxes with select controls', async () => {
        ...
        const checkbox = f.root.querySelector<HTMLInputElement>('input[aria-label="Mixed technology"]')!.getBoundingClientRect();
    ...
    it('repairs individual armor facings and structure while locked, retaining other damage and undo history', async () => {
        ...
        const rear = [...index.armorFaces.values()].find(face => face.locationId === location.id && face.face === 'rear')!;
        ...
        ] satisfies CBTUnitCommand[]) await f.force.dispatchUnitCommand(member.id, command);
        f.view.detectChanges();
        await f.view.whenStable();
    ...
    it('denies read-only defense repairs and hides runtime structure bars for standalone designs', async () => {
        ...
        const index = f.instance.getIndex();
        const location = [...index.locations.values()].find(location => location.code === 'LT')!;
        const face = [...index.armorFaces.values()].find(face => face.locationId === location.id && face.face === 'front')!;
        await f.force.dispatchUnitCommand(f.editor.forceMember()!.id, { type: 'damage-armor', faceId: face.id, amount: 2, target: 'committed' });
        await f.force.dispatchUnitCommand(f.editor.forceMember()!.id, { type: 'damage-internal', locationId: location.id, amount: 1, target: 'committed' });
    ...
    it('opens the canonical metric breakdowns from the header and weight summary', async () => {
        ...
        for (const [kind, total] of [['bv', f.editor.entity().battleValue()], ['cost', f.editor.entity().cost()], ['weight', f.editor.mass()]] as const) {
    ...
    it('previews the surrounding blocks in their committed order when dragging a block downward', async () => {
        ...
        f.editor.onDrop({ item: { data: { equipmentId: ac.id, mountId: gun.mountId, sourceLocation: 'LT' } }, container: { data: { location: 'LT' } } });
        f.editor.endDrag();
        f.view.detectChanges();
        expect(f.editor.entity().equipment().find(m => m.mountId === bin.mountId)!.placements![0].slotIndex).toBe(0);
        expect(f.editor.entity().equipment().find(m => m.mountId === gun.mountId)!.placements![0].slotIndex).toBe(1);
    });
    ...
    for (const source of ['warehouse', 'another location'] as const) {
        for (const position of ['top', 'middle'] as const) {
            it(`inserts equipment from ${source} at the ${position}, matching the preview and undoing as one edit`, async () => {
                ...
                expect(f.editor.entity().equipment().find(m => m.mountId === second.mountId)!.placements![0].slotIndex).toBe(4);
    ...
    it('preserves warehouse insertion through the actual CDK mouse-release event sequence', async () => {
        ...
        f.view.detectChanges();
        await f.view.whenStable();
        const source = [...f.root.querySelectorAll<HTMLElement>('.warehouse-item')]
            .find(row => row.querySelector('.equipment-label')?.textContent?.trim() === ac.name)!;
        const destination = f.root.querySelector<HTMLElement>(`[data-mount-id="${bin.mountId}"]`)!;
        destination.scrollIntoView({ block: 'center' });
        const from = source.getBoundingClientRect();
        const x = from.left + 20, y = from.top + 10;
        source.querySelector('.equipment-name')!.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, detail: 1, button: 0, buttons: 1, clientX: x, clientY: y }));
        document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: x + 15, clientY: y + 15 }));
        f.view.detectChanges();
        await f.view.whenStable();
        const preview = document.querySelector<HTMLElement>('.construction-drag-preview.cdk-drag-preview .installed-equipment')!;
        const installed = f.root.querySelector<HTMLElement>('[data-location="LT"] .installed-equipment')!;
        expect(preview).not.toBeNull();
        ...
        const endX = to.left + 10, endY = to.top + 2;
        ...
        expect(f.editor.entity().equipment().find(m => m.mountId === bin.mountId)!.placements![0].slotIndex).toBe(3);
    ...
    it('joins a transferred split block ahead of existing equipment when the whole weapon fits', async () => {
        ...
        expect(f.root.querySelector<HTMLElement>(`[data-mount-id="${bin.mountId}"]`)!.parentElement!.style.gridRow).toBe('11 / span 1');
        f.editor.onDrop({ item: { data }, container: { data: { location: 'RT' } } });
        f.editor.endDrag();
        expect(f.editor.selectedMount()?.placements).toEqual(Array.from({ length: 10 }, (_, slotIndex) => ({ location: 'RT', slotIndex })));
        expect(f.editor.entity().equipment().find(m => m.mountId === bin.mountId)!.placements![0].slotIndex).toBe(10);
    });
    ...
    it('keeps the docked block dimensions, style and bottom-right grab point in the floating preview', async () => {
        ...
        const x = Math.round(rect.right - 8), y = Math.round(rect.bottom - 8);
        block.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, detail: 1, button: 0, buttons: 1, clientX: x, clientY: y }));
        document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: x + 15, clientY: y + 15 }));
        f.view.detectChanges();
        await f.view.whenStable();
        const preview = document.querySelector<HTMLElement>('.construction-drag-preview.cdk-drag-preview .installed-equipment');
        try {
            ...
            document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: x + 30, clientY: y + 30 }));
    ...
    it('uses the installed block appearance and relative grab point when dragging unallocated equipment', async () => {
        const f = await create();
        f.editor.entity().addEquipment({ equipmentId: ac.id, equipment: ac, allocation: { kind: 'unallocated' },
            rearMounted: false, turretMounted: false, omniPodMounted: false, armored: false });
        f.view.detectChanges();
        await f.view.whenStable();
        ...
        source.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0, buttons: 1, clientX: x, clientY: y }));
        source.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, detail: 1, button: 0, buttons: 1, clientX: x, clientY: y }));
        document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: x + 15, clientY: y + 15 }));
        f.view.detectChanges();
        await f.view.whenStable();
        try {
    ...
    it('puts ordering arrows in the inspector heading and avoids duplicate actuator controls', async () => {
        ...
        f.root.querySelector<HTMLButtonElement>('[data-location="LT"] .mounted-name')!.click();
        f.view.detectChanges();
        expect(f.root.querySelector('.installed-inspector .equipment-details')?.textContent?.replace(/\s+/g, ' ')).toContain('BV:50');
        ...
        expect(arrows.compareDocumentPosition(f.root.querySelector('.equipment-details')!) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
        ...
        expect(f.root.querySelector('.installed-inspector [title="Disable this actuator"]')?.textContent?.trim()).toBe('UNINSTALL');
    });
    ...
    it('orders only the inspected location of split equipment and shows compact occupied locations', async () => {
        ...
        const bin = f.editor.selectedMount()!;
        f.view.detectChanges();
        f.root.querySelector<HTMLButtonElement>(`[data-location="RA"] [data-mount-id="${gun.mountId}"] .mounted-name`)!.click();
        f.view.detectChanges();
        expect(f.root.querySelector('.installed-inspector .muted')?.textContent?.replace(/\s+/g, ' ')).toContain('Installed · RT/RA ·');
        const armArrows = [...f.root.querySelectorAll<HTMLButtonElement>('.mount-order-controls button')];
        expect(armArrows.map(button => button.getAttribute('aria-label'))).toEqual(['Move up in Right Arm', 'Move down in Right Arm']);
        expect(armArrows.every(button => button.disabled)).toBeTrue();
        f.editor.closeInstalledInspector();
        f.view.detectChanges();
        f.root.querySelector<HTMLButtonElement>(`[data-location="RT"] [data-mount-id="${gun.mountId}"] .mounted-name`)!.click();
        f.view.detectChanges();
        ...
        expect(f.editor.selectedMount()!.placements!.filter(placement => placement.location === 'RA')).toEqual(armPlacements);
    ...
    it('reorders a fixed Omni split block within its secondary body location and rejects moving it elsewhere', async () => {
        const f = await create();
        f.editor.setField(f.editor.fields().find(field => field.id === 'omni')!, true);
        f.editor.install(splitAc, 'RA');
    ...
    it('reorders Omni material slots within a location while locking each location count', async () => {
        ...
        const data = { equipmentId: endo.id, mountId: mount.mountId, sourceLocation: 'LT', sourceSlotIndex: 3, slotCount: 3 };
        f.editor.startDrag(data);
        f.editor.dragTarget.set({ location: 'LT' });
    ...
    it('expands split allocation on demand and keeps the inspected location after applying edits', async () => {
        ...
        expect(f.root.querySelector('.mount-order-controls button')?.getAttribute('aria-label')).toBe('Move up in Right Arm');
        expect(split.open).toBeTrue();
        split.querySelector('summary')!.click();
        expect(split.open).toBeFalse();
        split.querySelector('summary')!.click();
    ...
    it('joins a dragged eight-slot arm block with its two-slot torso block only when all ten slots fit', async () => {
        ...
        f.editor.endDrag();
        expect(f.editor.status()).toBe('');
        const joined = f.editor.entity().equipment().find(m => m.mountId === gun.mountId)!;
        expect(joined.placements?.map(p => p.location)).toEqual(Array(10).fill('RT'));
        f.editor.undo();
        f.editor.install(ac, 'RT');
        const restored = f.editor.entity().equipment().find(m => m.equipmentId === splitAc.id)!;
        ...
        f.editor.endDrag();
        expect(f.editor.entity().equipment().find(m => m.mountId === restored.mountId)?.placements).toEqual(restored.placements);
    });
    ...
    for (const material of ['endo', 'stealth'] as const) {
        it(`hides and rejects component armor for ${material}, ammunition, and integral heat sinks`, async () => {
            ...
            for (const id of [material === 'endo' ? endo.id : stealth.id, ammo.id, sinks.id]) {
                ...
                f.editor.inspector.open(f.root);
                f.view.detectChanges();
                expect([...f.root.querySelectorAll('.mount-controls label')].some(label => label.textContent?.includes('Armored component'))).toBeFalse();
    ...
    it('lets an illegal armored material be corrected without allowing armor to be added', async () => {
        ...
        const entity = f.editor.entity();
        entity.updateEquipment(mounts => mounts.map(mount => mount.equipmentId === endo.id ? mount.clone({ armored: true }) : mount));
        f.editor.selectMount(entity.equipment().find(mount => mount.equipmentId === endo.id)!);
        f.editor.inspector.open(f.root);
        f.view.detectChanges();
        await f.view.whenStable();
        const label = [...f.root.querySelectorAll('.mount-controls label')].find(item => item.textContent?.includes('Armored component'))!;
        ...
        expect([...f.root.querySelectorAll('.mount-controls label')].some(item => item.textContent?.includes('Armored component'))).toBeFalse();
    });
    ...
    for (const fieldId of ['engineRating', 'walkMP'] as const) {
        it(`unallocates equipment displaced by ${fieldId} changes and restores the layout on undo`, async () => {
            const f = await create();
            f.editor.setField(f.editor.fields().find(field => field.id === 'engineRating')!, 300);
            f.editor.install(srm, 'CT');
            const mount = f.editor.entity().equipment().find(mount => mount.equipmentId === srm.id)!;
            const placements = [{ location: 'CT', slotIndex: 10 }, { location: 'CT', slotIndex: 11 }];
            expect(mount.placements).toEqual(placements);
            f.editor.setField(f.editor.fields().find(field => field.id === fieldId)!, fieldId === 'engineRating' ? 500 : 10);
            f.view.detectChanges();
            await f.view.whenStable();
            ...
            expect((f.editor.entity() as MekEntity).criticalSlotGrid().get('CT')!.slice(10, 12).every(slot => slot.type === 'system')).toBeTrue();
            expect(f.editor.entity().validationResult().messages.some(message => message.code === 'CRIT_PLACEMENT_CONFLICT')).toBeFalse();
            f.editor.undo();
            expect(f.editor.entity().mountedEngine().rating).toBe(300);
            expect(f.editor.entity().equipment().find(item => item.equipmentId === srm.id)?.placements).toEqual(placements);
            expect(f.editor.unallocated()).toEqual([]);
            f.editor.redo();
            expect(f.editor.entity().mountedEngine().rating).toBe(500);
    ...
    it('links the engine selector to Walk MP and undoes both values together', async () => {
        ...
        f.view.detectChanges();
        await f.view.whenStable();
        const selector = f.root.querySelector<HTMLSelectElement>('.installed-inspector select[aria-label="Engine rating"]')!;
        ...
        expect(f.editor.entity().mountedEngine().rating).toBe(350);
        expect(f.editor.entity().originalWalkMP()).toBe(7);
        f.editor.setField(f.editor.fields().find(field => field.id === 'walkMP')!, 6);
        expect(f.editor.entity().mountedEngine().rating).toBe(300);
    ...
    it('opens a separate installed inspector, marks armor, and exposes system-slot armor and actuator removal', async () => {
        ...
        f.editor.closeInstalledInspector();
        f.view.detectChanges(); await f.view.whenStable();
        ...
        f.editor.closeInstalledInspector();
        f.view.detectChanges(); await f.view.whenStable();
        const remove = f.root.querySelector<HTMLButtonElement>('[aria-label="Uninstall Lower Arm Actuator from Right Arm"]')!;
        expect(remove).not.toBeNull();
        remove.scrollIntoView({ block: 'center', inline: 'center' });
        const removeBounds = remove.getBoundingClientRect();
        const removeHit = document.elementFromPoint(removeBounds.left + removeBounds.width / 2, removeBounds.top + removeBounds.height / 2) as HTMLElement;
    ...
    it('delays equipment and system hover, allows crossing to the panel, pins on click, and closes on CDK drag', async () => {
        ...
        mount.dispatchEvent(new PointerEvent('pointerdown', { pointerType: 'mouse', bubbles: true, button: 0, buttons: 1, clientX: x, clientY: y }));
        mount.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, detail: 1, button: 0, buttons: 1, clientX: x, clientY: y }));
        document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: x + 10, clientY: y + 10 }));
        f.view.detectChanges();
    ...
    it('shows default shots in the equipment list and configured shots in installed ammo slots', async () => {
        ...
        f.view.detectChanges();
        expect([...f.root.querySelectorAll('.warehouse-item .equipment-name span')].map(element => element.textContent)).toContain('Test AC Ammo (20)');
    ...
    it('keeps loadout labels, inspector titles and record-sheet names in sync when systems change', async () => {
        ...
        for (const [system, location, fieldId, value, label] of [
            ...
            f.view.detectChanges();
            f.editor.setField(f.editor.systemFields().find(field => field.id === fieldId)!, value);
            f.view.detectChanges();
            await f.view.whenStable();
            expect(f.editor.status()).toBe('');
            expect(f.root.querySelector('.installed-inspector h3 > span')?.textContent).toBe(label);
            const buttons = [...f.root.querySelectorAll<HTMLButtonElement>('.system-name')]
                .filter(button => button.querySelector('.equipment-label')?.textContent === label);
            expect(buttons.length).toBeGreaterThan(0);
            expect(buttons.every(button => button.title === `Inspect ${label}`)).toBeTrue();
            const entity = f.editor.entity() as MekEntity;
            const slot = entity.criticalSlotGrid().get(location)!.find(slot => slot.type === 'system' && slot.systemType === system);
    ...
    it('keeps engine sections separate around a four-slot gyro and embeds integral sinks once', async () => {
        ...
        expect(rows.slice(0, 3).map(row => [row.system, row.index, row.span])).toEqual([
            ['Engine', 0, 3], ['Gyro', 3, 4], ['Engine', 7, 3],
        ]);
        ...
        const badgeHit = document.elementFromPoint(badgeBounds.left + badgeBounds.width / 2, badgeBounds.top + badgeBounds.height / 2) as HTMLElement;
    ...
    it('marks only hit ticks and destroys every engine block on the third hit, including preview damage', async () => {
        ...
        expect([...f.systems('Engine')[0].querySelectorAll('i')].map(tick => tick.classList.contains('destroyed-tick'))).toEqual([false, true, false]);
        await f.hit('CT', 7, 'pending');
        expect(f.systems('Engine').some(block => block.classList.contains('destroyed-equipment'))).toBeFalse();
        expect([...f.systems('Engine')[1].querySelectorAll('i')].map(tick => tick.classList.contains('destroyed-tick'))).toEqual([true, false, false]);
    ...
    for (const ruleset of ['core-2026', 'total-warfare'] as const) {
        it(`uses ${ruleset} autocannon failure rules without changing per-slot ticks`, async () => {
            ...
            expect([...block().querySelectorAll('i')].map(tick => tick.classList.contains('destroyed-tick'))).toEqual([false, true, false]);
        ...
        it(`uses ${ruleset} heavy-duty gyro thresholds`, async () => {
            ...
            const gyro = f.editor.locations().find(location => location.id === 'CT')!.rows.find(row => row.system === 'Gyro')!;
    ...
    it('omits mass already accounted for by armor from critical-slot material entries', async () => {
        const f = await create();
        const ferro = new MiscEquipment({ id: 'ISHeavyFerroFibrous', name: 'Heavy Ferro-Fibrous', type: 'misc',
            flags: ['F_HEAVY_FERRO', 'F_MEK_EQUIPMENT'], stats: { criticalSlots: 21, tonnage: 0 } });
        const mount = f.editor.entity().addEquipment({ equipmentId: ferro.id, equipment: ferro,
            allocation: { kind: 'location', location: 'LA', placements: [4, 5, 6, 7].map(slotIndex => ({ location: 'LA', slotIndex })) },
            rearMounted: false, turretMounted: false, omniPodMounted: false, armored: false });
    ...
    it('keeps noncontiguous equipment blocks separate and follows the hit slot when a mount moves', async () => {
        const f = await create();
        const mount = f.editor.entity().equipment().find(mount => mount.equipmentId === ac.id)!;
        f.editor.entity().moveEquipment(mount, 'LT', [0, 1, 4].map(slotIndex => ({ location: 'LT', slotIndex })));
        await f.hit('LT', 2);
        ...
        f.editor.entity().moveEquipment(mount, 'LT', [7, 8, 9].map(slotIndex => ({ location: 'LT', slotIndex })));
        await f.view.whenStable();
        expect(blocks().length).toBe(1);
        expect([...blocks()[0].querySelectorAll('i')].map(tick => tick.classList.contains('destroyed-tick'))).toEqual([false, false, true]);
