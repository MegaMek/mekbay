...
describe('force Omni design boundary', () => {
  ...
  const equipment = new MiscEquipment({
    ...
    type: 'misc',
    tech,
    flags: ['F_MEK_EQUIPMENT', 'F_TANK_EQUIPMENT'],
  ...
  const sink = new MiscEquipment({
    ...
    type: 'misc',
    tech,
    flags: ['F_HEAT_SINK', 'F_MEK_EQUIPMENT'],
    stats: { tonnage: 1, criticalSlots: 1 },
  });
  ...
  const jet = new MiscEquipment({
    ...
    type: 'misc',
    tech,
    flags: ['F_JUMP_JET', 'F_MEK_EQUIPMENT'],
    stats: { tonnage: 0.5, criticalSlots: 1 },
  });
  const registry = createTestEquipmentRegistry(
    Object.fromEntries([equipment, sink, double, jet].map((item) => [item.id, item])),
  );
  ...
  it('allows fixed slots to shift within their body location but detects removal and relocation', () => {
    ...
    entity.updateEquipment((mounts) =>
      mounts.map((mount) =>
        mount.mountId === fixed.mountId
          ? mount.clone({ omniPodMounted: false })
          : mount.mountId === rear.mountId
            ? mount.clone({ omniPodMounted: false, rearMounted: true })
            : mount,
      ),
    );
    const baseline = constructionOmniBaseSource(entity);
  ...
  it('round-trips real pod sinks and jump jets without treating derived counts as base edits', () => {
    ...
    expect(constructionReconfigurationIssues(entity)).toContain(
      'Pod heat sinks must use the fixed chassis heat-sink type.',
    );
  });
  ...
  it('inherits a vehicle turret capacity even when its original file used automatic turret mass', () => {
    ...
    expect(constructionReconfigurationIssues(entity)).toContain(
      'Turret equipment requires 1 t of turret structure; the base chassis provides 0.5 t.',
    );
  });
});
